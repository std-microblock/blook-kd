...
VOID KDUTestLoad() {
    ...
    for (i = 0; i < provLoadData->NumberOfEntries; i++) {
        pvData = KDULoadResource(provLoadData->Entries[i].ResourceId, hProv,
                                 &dataSize, PROVIDER_RES_KEY, TRUE);
...
VOID KDUTestDSE(PKDU_CONTEXT Context) {
    ...
    if (prov->Callbacks.ReadKernelVM) {
        prov->Callbacks.ReadKernelVM(Context->DeviceHandle, g_CiOptions,
                                     &oldValue, sizeof(oldValue));
        Beep(0, 0);
    }

    if (prov->Callbacks.WriteKernelVM) {
        prov->Callbacks.WriteKernelVM(Context->DeviceHandle, g_CiOptions,
...
BOOL WINAPI TestPhysMemEnumCallback(_In_ ULONG_PTR Address,
    ...
    if (Params->ReadPhysicalMemory(Params->DeviceHandle, Address, &buffer,
                                   PAGE_SIZE)) {
        if (signatureSize ==
            RtlCompareMemory(
                Params->DispatchSignature,
...
VOID TestSymbols() {
    if (symInit()) {
        ...
        HMODULE hModule =
            LoadLibraryEx(NTOSKRNL_EXE, NULL, DONT_RESOLVE_DLL_REFERENCES);
...
VOID TestSuperfetch(PKDU_CONTEXT Context) {
    ...
    RtlAdjustPrivilege(SE_PROF_SINGLE_PROCESS_PRIVILEGE, TRUE, FALSE,
                       &oldValue);
...
VOID TestSuperfetchWithDriver(PKDU_CONTEXT Context) {
    ...
    if (!supBuildSuperfetchMemoryMap(&memoryMap)) {
        supPrintfEvent(kduEventError, "[-] Failed to build memory map\n");
        return;
    }
    ...
    ntosBase = supGetNtOsBase();
    supPrintfEvent(kduEventInformation, "[+] ntoskrnl base: 0x%llX\n",
                   ntosBase);

    if (!supSuperfetchVirtualToPhysical(&memoryMap, ntosBase, &physAddress)) {
        supPrintfEvent(kduEventError, "[-] Translation failed\n");
        supFreeSuperfetchMemoryMap(&memoryMap);
        return;
    }

    supPrintfEvent(kduEventInformation, "[+] Translated to physical: 0x%llX\n",
                   physAddress);

    //
    // Read MZ signature via physical memory
    //
    if (prov->Callbacks.ReadPhysicalMemory(Context->DeviceHandle, physAddress,
                                           &dosSignature,
                                           sizeof(dosSignature))) {
        if (dosSignature == IMAGE_DOS_SIGNATURE) {
            ...
            supPrintfEvent(kduEventError, "[-] MZ signature mismatch: 0x%04X\n",
                           dosSignature);
        }
    } else {
        supPrintfEvent(kduEventError, "[-] Failed to read physical memory\n");
    }
    ...
    //
    dosSignature = 0;
    if (prov->Callbacks.ReadKernelVM(Context->DeviceHandle, ntosBase,
                                     &dosSignature, sizeof(dosSignature))) {
        if (dosSignature == IMAGE_DOS_SIGNATURE) {
            supPrintfEvent(
                kduEventInformation,
        ...
        supPrintfEvent(kduEventError, "[-] Failed to read virtual memory\n");
    }

    supFreeSuperfetchMemoryMap(&memoryMap);

    supPrintfEvent(kduEventInformation, "[+] All tests completed\n");
}
...
VOID KDUTest() {
    ...
    // KDUTestLoad();
    // TestSymbols();
    Context = KDUProviderCreate(KDU_PROVIDER_TPUP, FALSE, NT_WIN10_20H1,
                                KDU_SHELLCODE_V1, ActionTypeMapDriver);
