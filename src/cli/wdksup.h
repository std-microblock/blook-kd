/* Header file for NT WDK definitions. WARNING: some structures are opaque and
 * incomplete. */
#pragma once
...
#define CM_RESOURCE_MEMORY_LARGE                                 \
    (CM_RESOURCE_MEMORY_LARGE_40 | CM_RESOURCE_MEMORY_LARGE_48 | \
...
typedef VOID WORKER_THREAD_ROUTINE(_In_ PVOID Parameter);

typedef WORKER_THREAD_ROUTINE* PWORKER_THREAD_ROUTINE;
typedef VOID* PACCESS_STATE;

typedef NTSTATUS DRIVER_INITIALIZE(_In_ struct _DRIVER_OBJECT* DriverObject,
...
typedef NTSTATUS(WINAPI* pfnIoCreateDriver)(_In_ PUNICODE_STRING DriverName,
                                            OPTIONAL _In_ PDRIVER_INITIALIZE
                                                InitializationFunction);

typedef VOID(NTAPI* pfnExQueueWorkItem)(_Inout_ PWORK_QUEUE_ITEM WorkItem,
                                        _In_ WORK_QUEUE_TYPE QueueType);

typedef NTSTATUS(NTAPI* pfnZwOpenSection)(_Out_ PHANDLE SectionHandle,
...
typedef NTSTATUS(NTAPI* pfnZwUnmapViewOfSection)(_In_ HANDLE ProcessHandle,
                                                 _In_opt_ PVOID BaseAddress);

typedef ULONG(NTAPI* pfnDbgPrint)(_In_ PCHAR Format, ...);

typedef PVOID(NTAPI* pfnExAllocatePool)(_In_ POOL_TYPE PoolType,
                                        _In_ SIZE_T NumberOfBytes);

typedef PVOID(NTAPI* pfnExAllocatePoolWithTag)(_In_ POOL_TYPE PoolType,
                                               _In_ SIZE_T NumberOfBytes,
                                               _In_ ULONG Tag);

typedef VOID(NTAPI* pfnExFreePoolWithTag)(_In_ PVOID P, _In_ ULONG Tag);
...
typedef NTSTATUS(NTAPI* pfnZwClose)(_In_ HANDLE Handle);

typedef VOID(NTAPI* pfnIofCompleteRequest)(_In_ VOID* Irp,
...
typedef VOID(NTAPI* pfnObfDereferenceObject)(_In_ PVOID Object);

typedef NTSTATUS(NTAPI* pfnKeSetEvent)(_In_ PKEVENT Event,
                                       _In_ KPRIORITY Increment,
                                       _In_ _Literal_ BOOLEAN Wait);

typedef NTSTATUS(NTAPI* pfnObCreateObject)(_In_ KPROCESSOR_MODE ProbeMode,
...
typedef VOID(NTAPI* pfnObMakeTemporaryObject)(_In_ PVOID Object);

typedef NTSTATUS(NTAPI* pfnZwMakeTemporaryObject)(_In_ HANDLE Handle);
