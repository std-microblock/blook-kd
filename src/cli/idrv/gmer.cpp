 ...
 *
 */
BOOL WINAPI GmerRegisterDriver(_In_ HANDLE DeviceHandle, _In_opt_ PVOID Param) {
    ...
    bResult =
        supCallDriver(DeviceHandle, IOCTL_GMER_REGISTER_CLIENT, &ulRegistration,
                      sizeof(ULONG), &ulRegistration, sizeof(ULONG));
 ...
 *
 */
BOOL WINAPI GmerReadVirtualMemory(_In_ HANDLE DeviceHandle,
    ...
    return supCallDriver(DeviceHandle, IOCTL_GMER_READVM, &request,
                         sizeof(GMER_READ_REQUEST), Buffer, NumberOfBytes);
}
 ...
 *
 */
BOOL WINAPI GmerWriteVirtualMemory(_In_ HANDLE DeviceHandle,
                                   _In_ ULONG_PTR VirtualAddress,
                                   _In_reads_bytes_(NumberOfBytes) PVOID Buffer,
                                   _In_ ULONG NumberOfBytes) {
    BOOL bResult = FALSE;
    ...
    pRequest = (GMER_WRITE_REQUEST*)supAllocateLockedMemory(
    ...
    if (pRequest) {
        ...
        bResult = supCallDriver(DeviceHandle, IOCTL_GMER_WRITEVM, pRequest,
                                (ULONG)size, NULL, 0);
