 ...
 *
 */
BOOL AsrEncryptDriverRequest(_In_ PUCHAR DriverRequest,
    ...
    do {
        ...
        if (!NT_SUCCESS(BCryptOpenAlgorithmProvider(
                &hAlgAes, BCRYPT_AES_ALGORITHM, NULL, 0))) {
            break;
        }

        if (!NT_SUCCESS(BCryptGenerateSymmetricKey(
                hAlgAes, &hKey, NULL, 0, encKey, sizeof(encKey), 0))) {
            break;
        }

        PUCHAR pbIv =
            (PUCHAR)HeapAlloc(heapCNG, HEAP_ZERO_MEMORY, request.SizeOfIv);
        if (pbIv) {
            ...
            pbCipherData =
                (PBYTE)HeapAlloc(heapCNG, HEAP_ZERO_MEMORY, cbCipherData);
            if (pbCipherData) {
                status =
                    BCryptEncrypt(hKey, DriverRequest, RequestSize, NULL, pbIv,
                                  request.SizeOfIv, pbCipherData, cbCipherData,
    ...
    if (bResult && cbResult && pbCipherData) {
        ULONG outSize =
            sizeof(ASRDRV_REQUEST) + cbResult + sizeof(ASRDRV_REQUEST_FOOTER);
        ...
        if (result) {
            ...
            RtlCopyMemory(RtlOffsetToPointer(result, sizeof(ASRDRV_REQUEST)),
                          pbCipherData, cbResult);

            requestFooter = (ASRDRV_REQUEST_FOOTER*)RtlOffsetToPointer(
                result, outSize - sizeof(ASRDRV_REQUEST_FOOTER));
 ...
 *
 */
BOOL AsrCallDriver(_In_ HANDLE DeviceHandle,
    ...
    if (!AsrEncryptDriverRequest((PUCHAR)&command, sizeof(command),
                                 &pvEncryptedCommand, &cbEncryptedCommand)) {
        return FALSE;
    }
    ...
    NTSTATUS status = supCallDriverEx(DeviceHandle, IOCTL_ASRDRV_EXEC_DISPATCH,
                                      pvEncryptedCommand, cbEncryptedCommand,
                                      &outBuffer, sizeof(outBuffer), &ioStatus);
 ...
 *
 */
BOOL WINAPI AsrReadPhysicalMemory(_In_ HANDLE DeviceHandle,
    ...
    return AsrCallDriver(DeviceHandle, IOCTL_ASRDRV_READ_MEMORY, &args);
}
 ...
 *
 */
BOOL WINAPI AsrWritePhysicalMemory(_In_ HANDLE DeviceHandle,
                                   _In_ ULONG_PTR PhysicalAddress,
                                   _In_ PVOID Buffer,
                                   _In_ ULONG NumberOfBytes) {
    ASRDRV_ARGS args;
    ...
    return AsrCallDriver(DeviceHandle, IOCTL_ASRDRV_WRITE_MEMORY, &args);
}
 ...
 *
 */
BOOL WINAPI RweReadPhysicalMemory(_In_ HANDLE DeviceHandle,
                                  _In_ ULONG_PTR PhysicalAddress,
                                  _In_ PVOID Buffer,
                                  _In_ ULONG NumberOfBytes) {
    BOOL bResult = FALSE;
    ...
    PVOID data = (ASR_RWE_REQUEST*)supAllocateLockedMemory(
    ...
    if (data) {
        ...
        if (supCallDriver(DeviceHandle, g_AsrReadPhysIOCTL, &request,
                          sizeof(request), &request, sizeof(request))) {
            RtlCopyMemory(
                Buffer, RtlOffsetToPointer(data, PhysicalAddress - mapAddress),
                size);
            bResult = TRUE;
        }
 ...
 *
 */
BOOL WINAPI RweWritePhysicalMemory(_In_ HANDLE DeviceHandle,
                                   _In_ ULONG_PTR PhysicalAddress,
                                   _In_ PVOID Buffer,
                                   _In_ ULONG NumberOfBytes) {
    ASR_RWE_REQUEST request;
    ...
    return supCallDriver(DeviceHandle, g_AsrWritePhysIOCTL, &request,
                         sizeof(request), &request, sizeof(request));
}
 ...
 *
 */
BOOL WINAPI AsrRegisterDriver(_In_ HANDLE DeviceHandle, _In_opt_ PVOID Param) {
