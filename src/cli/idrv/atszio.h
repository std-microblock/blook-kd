...
#define IOCTL_ATSZIO_MAP_USER_PHYSICAL_MEMORY                                \
    CTL_CODE(ATSZIO_DEVICE_TYPE, ATSZIO_MAP_SECTION_FUNCID, METHOD_BUFFERED, \
             FILE_ANY_ACCESS)  // 0x8807200C

#define IOCTL_ATSZIO_UNMAP_USER_PHYSICAL_MEMORY                                \
    CTL_CODE(ATSZIO_DEVICE_TYPE, ATSZIO_UNMAP_SECTION_FUNCID, METHOD_BUFFERED, \
             FILE_ANY_ACCESS)  // 0x88072010

#pragma warning(push)
#pragma warning(disable : 4324)  // structure padded due to __declspec(align())
typedef struct DECLSPEC_ALIGN(MEMORY_ALLOCATION_ALIGNMENT)
    _ATSZIO_PHYSICAL_MEMORY_INFO {
    ULONG_PTR Unused0;
