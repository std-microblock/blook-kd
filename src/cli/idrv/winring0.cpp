 ...
 *
 */
BOOL WRZeroReadPhysicalMemory(_In_ HANDLE DeviceHandle,
    ...
    return supCallDriver(DeviceHandle, IOCTL_OLS_READ_MEMORY, &request,
                         sizeof(OLS_READ_MEMORY_INPUT), Buffer, NumberOfBytes);
}
 ...
 *
 */
BOOL WINAPI WRZeroWritePhysicalMemory(_In_ HANDLE DeviceHandle,
                                      _In_ ULONG_PTR PhysicalAddress,
    ...
    pRequest = (OLS_WRITE_MEMORY_INPUT*)supAllocateLockedMemory(
    ...
    if (pRequest) {
        ...
        bResult = supCallDriver(DeviceHandle, IOCTL_OLS_WRITE_MEMORY, pRequest,
                                (ULONG)size, NULL, 0);
 ...
 *
 */
BOOL WINAPI WRZeroQueryPML4Value(_In_ HANDLE DeviceHandle,
    ...
    *Value = 0;
    ...
    do {
        ...
        for (ULONG_PTR i = 0; i < cbSize; i += PAGE_SIZE) {
            if (!WRZeroReadPhysicalMemory(DeviceHandle, i,
 ...
 *
 */
BOOL WINAPI WRZeroVirtualToPhysical(_In_ HANDLE DeviceHandle,
                                    _In_ ULONG_PTR VirtualAddress,
                                    _Out_ ULONG_PTR* PhysicalAddress) {
    return PwVirtualToPhysical(DeviceHandle, WRZeroQueryPML4Value,
                               WRZeroReadPhysicalMemory, VirtualAddress,
                               PhysicalAddress);
}
 ...
 *
 */
BOOL WINAPI WRZeroReadKernelVirtualMemory(_In_ HANDLE DeviceHandle,
    ...
    bResult = WRZeroVirtualToPhysical(DeviceHandle, Address, &physicalAddress);

    if (bResult) {
        bResult = WRZeroReadPhysicalMemory(DeviceHandle, physicalAddress,
 ...
 *
 */
BOOL WINAPI WRZeroWriteKernelVirtualMemory(_In_ HANDLE DeviceHandle,
                                           _In_ ULONG_PTR Address,
                                           _Out_writes_bytes_(NumberOfBytes)
                                               PVOID Buffer,
                                           _In_ ULONG NumberOfBytes) {
    ...
    if (bResult) {
        bResult = WRZeroWritePhysicalMemory(DeviceHandle, physicalAddress,
                                            Buffer, NumberOfBytes);
    }
