...
#define IOCTL_HP_READ_VMEM                                  \
    CTL_CODE(HP_DEVICE_TYPE, HP_READ_VMEM, METHOD_BUFFERED, \
             FILE_READ_ACCESS)  // 0x8000603C

#define IOCTL_HP_WRITE_VMEM                                  \
    CTL_CODE(HP_DEVICE_TYPE, HP_WRITE_VMEM, METHOD_BUFFERED, \
             FILE_WRITE_ACCESS)  // 0x80006038
...
_Success_(return != FALSE) BOOL WINAPI
    HpEtdReadVirtualMemory(_In_ HANDLE DeviceHandle,
                           _In_ ULONG_PTR VirtualAddress,
                           _Out_writes_bytes_(NumberOfBytes) PVOID Buffer,
                           _In_ ULONG NumberOfBytes);

_Success_(return != FALSE) BOOL WINAPI
    HpEtdWriteVirtualMemory(_In_ HANDLE DeviceHandle,
                            _In_ ULONG_PTR VirtualAddress,
