 ...
 *
 */
PVOID DI64MapMemory(_In_ HANDLE DeviceHandle,
    ...
    *AllocatedMdl = NULL;
    ...
    if (supCallDriver(DeviceHandle, IOCTL_DIRECTIO_MAP_PHYSICAL_MEMORY,
 ...
 *
 */
VOID DI64UnmapMemory(_In_ HANDLE DeviceHandle,
    ...
    supCallDriver(DeviceHandle, IOCTL_DIRECTIO_UNMAP_PHYSICAL_MEMORY, &request,
 ...
 *
 */
BOOL WINAPI DI64QueryPML4Value(_In_ HANDLE DeviceHandle,
    ...
    *Value = 0;
    ...
    pbLowStub1M = (ULONG_PTR)DI64MapMemory(DeviceHandle, 0ULL, cbRead,
                                           &sectionHandle, &refObject, FALSE);
    ...
    if (pbLowStub1M) {
        ...
        DI64UnmapMemory(DeviceHandle, (PVOID)pbLowStub1M, sectionHandle,
                        refObject);
    }
 ...
 *
 */
BOOL WINAPI DI64ReadWritePhysicalMemory(_In_ HANDLE DeviceHandle,
                                        _In_ ULONG_PTR PhysicalAddress,
    ...
    mappedSection = DI64MapMemory(DeviceHandle, PhysicalAddress, NumberOfBytes,
                                  &sectionHandle, &allocMdl, DoWrite);
    ...
    if (mappedSection) {
        ...
        DI64UnmapMemory(DeviceHandle, mappedSection, sectionHandle, allocMdl);
 ...
 *
 */
BOOL WINAPI DI64ReadPhysicalMemory(_In_ HANDLE DeviceHandle,
                                   _In_ ULONG_PTR PhysicalAddress,
                                   _In_ PVOID Buffer,
                                   _In_ ULONG NumberOfBytes) {
    return DI64ReadWritePhysicalMemory(DeviceHandle, PhysicalAddress, Buffer,
                                       NumberOfBytes, FALSE);
}
 ...
 *
 */
BOOL WINAPI DI64WritePhysicalMemory(_In_ HANDLE DeviceHandle,
                                    _In_ ULONG_PTR PhysicalAddress,
                                    _In_reads_bytes_(NumberOfBytes)
                                        PVOID Buffer,
                                    _In_ ULONG NumberOfBytes) {
 ...
 *
 */
BOOL WINAPI DI64VirtualToPhysical(_In_ HANDLE DeviceHandle,
                                  _In_ ULONG_PTR VirtualAddress,
                                  _Out_ ULONG_PTR* PhysicalAddress) {
    return PwVirtualToPhysical(DeviceHandle, DI64QueryPML4Value,
                               DI64ReadPhysicalMemory, VirtualAddress,
                               PhysicalAddress);
}
 ...
 *
 */
BOOL WINAPI DI64ReadKernelVirtualMemory(_In_ HANDLE DeviceHandle,
    ...
    bResult = DI64VirtualToPhysical(DeviceHandle, Address, &physicalAddress);

    if (bResult) {
        bResult = DI64ReadWritePhysicalMemory(DeviceHandle, physicalAddress,
 ...
 *
 */
BOOL WINAPI DI64WriteKernelVirtualMemory(_In_ HANDLE DeviceHandle,
                                         _In_ ULONG_PTR Address,
                                         _In_reads_bytes_(NumberOfBytes)
                                             PVOID Buffer,
                                         _In_ ULONG NumberOfBytes) {
