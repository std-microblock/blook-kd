...
typedef DWORD(WINAPI* pfnSymSetOptions)(_In_ DWORD SymOptions);

typedef BOOL(WINAPI* pfnSymInitialize)(_In_ HANDLE hProcess,
                                       _In_opt_ PCWSTR UserSearchPath,
                                       _In_ BOOL fInvadeProcess);

typedef DWORD64(WINAPI* pfnSymLoadModuleEx)(_In_ HANDLE hProcess,
...
typedef BOOL(WINAPI* pfnSymFromName)(_In_ HANDLE hProcess,
                                     _In_ PCSTR Name,
...
BOOL symLoadImageSymbols(_In_ LPCWSTR lpFileName,
    ...
    if (g_symInitialized) {
        ...
        bResult = (NULL != SymLoadModuleExProto(
                               NtCurrentProcess(), NULL, lpFileName, NULL,
                               (DWORD64)ImageBase, ImageSize, NULL, 0));
 ...
 *
 */
BOOL symLookupAddressBySymbol(_In_ LPCSTR SymbolName,
    ...
    if (g_symInitialized) {
        ...
        if (symbolInfo) {
            ...
            bResult =
                SymFromNameProto(NtCurrentProcess(), SymbolName, symbolInfo);
...
BOOL symInit() {
    ...
    do {
        ...
        if (cch == 0 || cch > MAX_PATH) {
            supShowWin32Error("[!] Cannot query current directory",
                              GetLastError());
            break;
        }
        ...
        if (g_hDbgHelp == NULL) {
            supShowWin32Error(
                "[!] Cannot load dbghelp.dll, make sure it is in program "
                "directory",
                GetLastError());
            break;
        }
        *lpEnd = 0;
        ...
        if (g_hSymSrv == NULL) {
            supShowWin32Error(
                "[!] Cannot load symsrv.dll, make sure it is in program "
                "directory",
                GetLastError());
        ...
        SymSetOptionsProto =
            (pfnSymSetOptions)GetProcAddress(g_hDbgHelp, "SymSetOptions");
        SymInitializeProto =
            (pfnSymInitialize)GetProcAddress(g_hDbgHelp, "SymInitializeW");

        SymLoadModuleExProto =
            (pfnSymLoadModuleEx)GetProcAddress(g_hDbgHelp, "SymLoadModuleExW");
        SymFromNameProto =
            (pfnSymFromName)GetProcAddress(g_hDbgHelp, "SymFromName");

        if (SymSetOptionsProto == NULL || SymInitializeProto == NULL ||
            SymLoadModuleExProto == NULL || SymFromNameProto == NULL) {
            supPrintfEvent(
                kduEventError,
        ...
        SymSetOptionsProto(SYMOPT_CASE_INSENSITIVE | SYMOPT_UNDNAME |
        ...
        if (cch > 0 && cch < MAX_PATH) {
            StringCchPrintf(
                szUserSearchPath, RTL_NUMBER_OF(szUserSearchPath),
                L"srv*%ws\\Symbols*https://msdl.microsoft.com/download/symbols",
                szTemp);

        } else {
            supShowWin32Error("[!] Cannot query temp directory",
                              GetLastError());
        ...
        bInitSuccess =
            SymInitializeProto(NtCurrentProcess(), szUserSearchPath, FALSE);
