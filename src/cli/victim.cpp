 ...
 *
 */
BOOL VpCreate(_Inout_ PKDU_VICTIM_PROVIDER Context,
    ...
    return Context->Callbacks.Create(
        ModuleBase, Context->Name, Context->ResourceId, Context->DesiredAccess,
        VictimHandle, &Context->Data.VictimImage, Callback, CallbackParam);
}
 ...
 *
 */
BOOL VpRelease(_In_ PKDU_VICTIM_PROVIDER Context,
 ...
 *
 */
VOID VpExecutePayload(_In_ PKDU_VICTIM_PROVIDER Context,
                      _Out_opt_ PHANDLE VictimHandle) {
    Context->Callbacks.Execute(Context->Name, Context->DesiredAccess,
                               VictimHandle);
}
 ...
 * This routine will try to force unload driver on loading if Force parameter
 * set to TRUE.
 *
 */
NTSTATUS VppLoadUnloadDriver(_In_ LPCWSTR Name,
    ...
    if (Unload) {
        ...
        ntStatus =
            supLoadDriverEx(Name, ImagePath, Force, Callback, CallbackParam);
    }
 ...
 *
 */
LPWSTR VppBuildDriverName(_In_ LPCWSTR VictimName) {
    LPWSTR lpFileName;
    ...
    if (lpFileName == NULL) {
        ...
        if (FAILED(StringCchPrintf(
                lpFileName, cchLength, L"%ws\\system32\\drivers\\%ws.sys",
                USER_SHARED_DATA->NtSystemRoot, VictimName))) {
 ...
 *
 */
BOOL VpCreateCallback(_In_ HINSTANCE ModuleBase,
    ...
    if (driverFileName) {
        do {
            if (supIsObjectExists((LPWSTR)L"\\Device", Name)) {
                ...
                supPrintfEvent(kduEventError, "[!] Attempt to unload %ws\r\n",
                               Name);

                ntStatus = VppLoadUnloadDriver(Name, driverFileName, FALSE,
                                               TRUE, NULL, NULL);

                if (!NT_SUCCESS(ntStatus)) {
                    supShowHardError("[!] Could not force unload victim, abort",
                                     ntStatus);
                    break;
                } else {
                    supPrintfEvent(
            ...
            drvBuffer = (PBYTE)KDULoadResource(
                ResourceId, ModuleBase, &resourceSize, PROVIDER_RES_KEY, TRUE);
            ...
            if (VictimImage) {
                ...
                printf_s(
                    "[+] Mapped victim image at %p with size 0x%lX bytes\r\n",
                    vpImage, vSize);
            ...
            writeBytes = (ULONG)supWriteBufferToFile(driverFileName, drvBuffer,
                                                     resourceSize, TRUE, FALSE,
                                                     &ntStatus);
            ...
            if (resourceSize != writeBytes) {
                ...
                if (ntStatus == STATUS_SHARING_VIOLATION) {
                    supPrintfEvent(kduEventError,
                                   "[!] Sharing violation, driver maybe in "
                                   "use, please close all application(s) that "
                                   "are using this driver\r\n");
                } else {
                    supShowHardError(
                        "[!] Could not extract victim driver, abort", ntStatus);
                }
            ...
            ntStatus = VppLoadUnloadDriver(Name, driverFileName, TRUE, FALSE,
            ...
            if (NT_SUCCESS(ntStatus)) {
                ...
                if (VictimHandle) {
                    ntStatus =
                        supOpenDriver(Name, DesiredAccess, &deviceHandle);
                    if (NT_SUCCESS(ntStatus)) {
 ...
 *
 */
BOOL VpReleaseCallback(_In_ LPCWSTR Name) {
    BOOL bResult = FALSE;

    LPWSTR driverFileName = VppBuildDriverName(Name);
    if (driverFileName) {
        bResult = NT_SUCCESS(
            VppLoadUnloadDriver(Name, driverFileName, FALSE, TRUE, NULL, NULL));
 ...
 *
 */
VOID VpExecuteCallback(_In_ LPCWSTR Name,
                       _In_ ACCESS_MASK DesiredAccess,
 ...
 *
 */
VOID VpExecuteCallbackEx(_In_ LPCWSTR Name,
                         _In_ ACCESS_MASK DesiredAccess,
                         _Out_ PHANDLE VictimHandle) {
    ...
    if (NT_SUCCESS(supOpenDriver(Name, DesiredAccess, &victimHandle))) {
        supCallDriver(victimHandle, 0xBADDAB, &dummy, sizeof(dummy), &dummy,
                      sizeof(dummy));
    }
 ...
 *
 */
BOOL VppOpenExistingDriverDevice(_In_ LPCWSTR Name,
                                 _In_ ACCESS_MASK DesiredAccess,
                                 _Out_opt_ PHANDLE VictimHandle) {
    HANDLE deviceHandle = NULL;
    NTSTATUS ntStatus;
    ...
    if (lpDeviceName) {
        StringCchPrintf(lpDeviceName, sz / sizeof(WCHAR), L"\\Device\\%ws",
                        Name);
 ...
 *
 */
VOID VpExecuteFromExistingCallback(_In_ LPCWSTR Name,
                                   _In_ ACCESS_MASK DesiredAccess,
                                   _Out_ PHANDLE VictimHandle) {
 ...
 *
 */
BOOL VpCreateFromExistingCallback(_In_ HINSTANCE ModuleBase,
                                  _In_ LPCWSTR Name,
                                  _In_ ULONG ResourceId,
                                  _In_ ACCESS_MASK DesiredAccess,
                                  _Out_opt_ PHANDLE VictimHandle,
    ...
    if (VictimImage) {
        *VictimImage = NULL;
        ...
        PBYTE drvBuffer = (PBYTE)KDULoadResource(
            ResourceId, ModuleBase, &resourceSize, PROVIDER_RES_KEY, TRUE);
 ...
 *
 */
BOOL VpReleaseCallbackStub(_In_ LPCWSTR Name) {
 ...
 *
 */
NTSTATUS CALLBACK VpLoadDriverCallback(_In_ PUNICODE_STRING RegistryPath,
...
_Success_(return != FALSE) BOOL
    VpQueryInformation(_In_ PKDU_VICTIM_PROVIDER Context,
    ...
    switch (VictimInformationClass) {
        case VictimImageInformation:
            ...
            if (InformationLength == sizeof(VICTIM_IMAGE_INFORMATION)) {
                ...
                sectionBase = ntsupLookupImageSectionByName(
                    (CHAR*)TEXT_SECTION, TEXT_SECTION_LEGNTH,
                    (PVOID)Context->Data.VictimImage, &sectionSize);

                if (sectionBase && sectionSize) {
                    PBYTE ptrCode = NULL;

                    ptrCode = (PBYTE)ntsupFindPattern(
                        (PBYTE)sectionBase, sectionSize,
                        (PBYTE)dispatchSignature, signatureSize);

                    if (ptrCode) {
                        imageInfo->DispatchOffset = (ULONG_PTR)ptrCode & 0xffff;
                        imageInfo->DispatchPageOffset =
                            imageInfo->DispatchOffset & 0xfff;

                        LONG_PTR rel =
                            (LONG_PTR)sectionBase - (LONG_PTR)ptrCode - 5;
        ...
        case VictimDriverInformation:
            ...
            if (InformationLength == sizeof(VICTIM_DRIVER_INFORMATION)) {
                ...
                PRTL_PROCESS_MODULES modulesList =
                    (PRTL_PROCESS_MODULES)supGetLoadedModulesList(FALSE, NULL);
                if (modulesList) {
                    ...
                    StringCchPrintf(szTargetDriver, MAX_PATH, L"%ws.sys",
                                    Context->Name);
                    ...
                    ntStatus = RtlUnicodeStringToAnsiString(
                        &driverNameAs, &driverNameUs, TRUE);
                    if (NT_SUCCESS(ntStatus) && driverNameAs.Buffer) {
                        target = (PRTL_PROCESS_MODULE_INFORMATION)
                            ntsupFindModuleEntryByName(modulesList,
                                                       driverNameAs.Buffer);
                        if (target) {
                            driverInfo->LoadedImageBase =
                                (ULONG_PTR)target->ImageBase;
