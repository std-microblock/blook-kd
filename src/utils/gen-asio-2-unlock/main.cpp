...
BOOL UpdateChecksum(_In_ LPCSTR lpFileName) {
    BOOL bResult = FALSE;
    ...
    __try {
        hFile = CreateFileA(lpFileName, GENERIC_READ | GENERIC_WRITE, 0, NULL,
...
VOID ProcessFile(_In_ LPCSTR lpFileName) {
    ...
    do {
        bInit = CryptAcquireContext(&hProv, NULL, MS_ENH_RSA_AES_PROV,
        ...
        if (bInit == FALSE) {
            printf_s(
                "[G] Failed to acquire context for Crypto API, error %lX\n",
                GetLastError());
            break;
        }
        ...
        if (!CryptImportKey(hProv, (BYTE*)&KeyBlob, sizeof(KeyBlob), NULL, 0,
                            &hKey)) {
        ...
        if (!CryptEncrypt(hKey, NULL, FALSE, 0, (BYTE*)Buffer, &bytesIO,
                          bytesIO)) {
    ...
    if (hRes) {
        if (!UpdateResourceA(hRes, (LPCSTR)RT_RCDATA, "ASUSCERT",
...
int main() {
    ...
    if (l > 0) {
        printf_s(
            "GenAsIo2Unlock v1.0 built at %s\n[G] Processing input file %s\n",
