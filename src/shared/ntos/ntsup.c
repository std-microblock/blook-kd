/* Native API support functions. Only ntdll-bound import. */
...
#define NTSUP_BSIG0(x) \
    (NTSUP_ROTR32(x, 2) ^ NTSUP_ROTR32(x, 13) ^ NTSUP_ROTR32(x, 22))
#define NTSUP_BSIG1(x) \
    (NTSUP_ROTR32(x, 6) ^ NTSUP_ROTR32(x, 11) ^ NTSUP_ROTR32(x, 25))
...
VOID ntsupSha256Transform(_Inout_ PNTSUP_SHA256_CTX Ctx,
    ...
    for (i = 16; i < 64; i++) {
        W[i] = NTSUP_SSIG1(W[i - 2]) + W[i - 7] + NTSUP_SSIG0(W[i - 15]) +
               W[i - 16];
    }
...
VOID ntsupSha256Init(_Out_ PNTSUP_SHA256_CTX Ctx) {
...
VOID ntsupSha256Update(_Inout_ PNTSUP_SHA256_CTX Ctx,
...
VOID ntsupSha256Final(_Inout_ PNTSUP_SHA256_CTX Ctx,
 ...
 *
 */
PVOID ntsupHeapAlloc(_In_ SIZE_T Size) {
 ...
 *
 */
VOID ntsupHeapFree(_In_ PVOID BaseAddress) {
 ...
 *
 */
PVOID ntsupVirtualAllocEx(_In_ SIZE_T Size,
    ...
    bufferSize = Size;
    ntStatus = NtAllocateVirtualMemory(NtCurrentProcess(), &bufferPtr, 0,
                                       &bufferSize, AllocationType, Protect);
 ...
 *
 */
PVOID ntsupVirtualAlloc(_In_ SIZE_T Size) {
 ...
 *
 */
BOOL ntsupVirtualLock(_In_ LPVOID lpAddress, _In_ SIZE_T dwSize) {
    return (NT_SUCCESS(NtLockVirtualMemory(NtCurrentProcess(), &lpAddress,
                                           &dwSize, MAP_PROCESS)));
}
 ...
 *
 */
BOOL ntsupVirtualUnlock(_In_ LPVOID lpAddress, _In_ SIZE_T dwSize) {
    return (NT_SUCCESS(NtUnlockVirtualMemory(NtCurrentProcess(), &lpAddress,
                                             &dwSize, MAP_PROCESS)));
}
 ...
 *
 */
BOOL ntsupVirtualFree(_In_ PVOID Memory) {
    NTSTATUS ntStatus = STATUS_UNSUCCESSFUL;
    SIZE_T sizeDummy = 0;

    if (Memory) {
        ntStatus = NtFreeVirtualMemory(NtCurrentProcess(), &Memory, &sizeDummy,
                                       MEM_RELEASE);
    } else {
 ...
 *
 */
SIZE_T ntsupWriteBufferToFile(_In_ PCWSTR FileName,
                              _In_ PVOID Buffer,
                              _In_ SIZE_T Size,
                              _In_ BOOL Flush,
                              _In_ BOOL Append,
    ...
    if (RtlDosPathNameToNtPathName_U(FileName, &ntFileName, NULL, NULL) ==
        FALSE) {
        if (Result)
    ...
    InitializeObjectAttributes(&attr, &ntFileName, OBJ_CASE_INSENSITIVE, 0,
                               NULL);

    __try {
        ntStatus = NtCreateFile(
            &hFile, desiredAccess, &attr, &ioStatus, NULL,
            FILE_ATTRIBUTE_NORMAL, 0, dwFlag,
        ...
        if (Size < 0x80000000) {
            ...
            ntStatus = NtWriteFile(hFile, 0, NULL, NULL, &ioStatus, ptr,
                                   blockSize, pPosition, NULL);
            if (!NT_SUCCESS(ntStatus))
                __leave;
 ...
 *
 */
PVOID ntsupFindModuleEntryByName(_In_ PRTL_PROCESS_MODULES ModulesList,
 ...
 *
 */
PVOID ntsupFindModuleEntryByName_U(_In_ PRTL_PROCESS_MODULES ModulesList,
    ...
    if (NT_SUCCESS(RtlInitUnicodeStringEx(&usString, ModuleName))) {
        ...
        if (NT_SUCCESS(
                RtlUnicodeStringToAnsiString(&moduleName, &usString, TRUE))) {
            for (i = 0; i < modulesCount; i++) {
                moduleEntry = &ModulesList->Modules[i];
                fnameOffset = moduleEntry->OffsetToFileName;
                entryName = (LPSTR)&moduleEntry->FullPathName[fnameOffset];
 ...
 *
 */
BOOL ntsupFindModuleEntryByAddress(_In_ PRTL_PROCESS_MODULES ModulesList,
                                   _In_ PVOID Address,
    ...
    *ModuleIndex = 0;
    ...
    for (i = 0; i < modulesCount; i++) {
        if (IN_REGION(Address, ModulesList->Modules[i].ImageBase,
 ...
 *
 */
PVOID ntsupGetModuleEntryByAddress(_In_ PRTL_PROCESS_MODULES ModulesList,
                                   _In_ PVOID Address) {
    ULONG i, modulesCount = ModulesList->NumberOfModules;
    ...
    for (i = 0; i < modulesCount; i++) {
        if (IN_REGION(Address, ModulesList->Modules[i].ImageBase,
            ...
            ntStatus = ntsupConvertToUnicode(
                (LPSTR)&moduleEntry
                    ->FullPathName[moduleEntry->OffsetToFileName],
                &usConvertedName);
            ...
            if (NT_SUCCESS(ntStatus)) {
                ...
                _strncpy(Buffer, ccBuffer, usConvertedName.Buffer, copyLength);
 ...
 *
 */
NTSTATUS ntsupConvertToUnicode(_In_ LPCSTR AnsiString,
 ...
 *
 */
NTSTATUS ntsupConvertToAnsi(_In_ LPCWSTR UnicodeString,
 ...
 *
 */
BOOLEAN ntsupEnablePrivilege(_In_ DWORD Privilege, _In_ BOOLEAN Enable) {
    ULONG returnLength;
    NTSTATUS ntStatus;
    HANDLE tokenHandle;
    ...
    if (NT_SUCCESS(ntStatus)) {
        ...
        ntStatus =
            NtAdjustPrivilegesToken(tokenHandle, FALSE, newState,
                                    sizeof(rawBuffer), NULL, &returnLength);
 ...
 *
 */
HANDLE ntsupGetCurrentProcessToken(VOID) {
    HANDLE tokenHandle = NULL;

    if (NT_SUCCESS(NtOpenProcessToken(NtCurrentProcess(), TOKEN_QUERY,
                                      &tokenHandle))) {
        return tokenHandle;
    }
    return NULL;
 ...
 *
 */
ULONG_PTR ntsupQuerySystemRangeStart(VOID) {
    NTSTATUS ntStatus;
    ULONG_PTR systemRangeStart = 0;
    ULONG memIO = 0;

    ntStatus = NtQuerySystemInformation(SystemRangeStartInformation,
                                        (PVOID)&systemRangeStart,
                                        sizeof(ULONG_PTR), &memIO);
...
BOOLEAN ntsupQueryUserModeAccessibleRange(
    ...
    ntStatus =
        NtQuerySystemInformation(SystemBasicInformation, (PVOID)&sysBasicInfo,
                                 sizeof(sysBasicInfo), &memIO);
 ...
 *
 */
BOOLEAN ntsupIsKdEnabled(_Out_opt_ PBOOLEAN DebuggerAllowed,
    ...
    ntStatus = NtQuerySystemInformation(SystemKernelDebuggerInformation,
                                        &kdInfo, sizeof(kdInfo), &returnLength);
    ...
    if (DebuggerAllowed) {
        ...
        ntStatus = NtQuerySystemInformation(SystemKernelDebuggerInformationEx,
                                            &kdInfoEx, sizeof(kdInfoEx),
                                            &returnLength);
 ...
 *
 */
BOOL ntsupIsProcess32bit(_In_ HANDLE hProcess) {
    ULONG returnLength;
 ...
 *
 */
PVOID ntsupGetLoadedModulesListEx(_In_ BOOL ExtendedOutput,
    ...
    infoClass =
        ExtendedOutput ? SystemModuleInformationEx : SystemModuleInformation;
    ...
    ntStatus =
        NtQuerySystemInformation(infoClass, buffer, bufferSize, &bufferSize);
    ...
    if (ntStatus == STATUS_INFO_LENGTH_MISMATCH) {
        ...
        ntStatus = NtQuerySystemInformation(infoClass, buffer, bufferSize,
                                            &bufferSize);
    }
 ...
 *
 */
PVOID ntsupGetLoadedModulesList(_Out_opt_ PULONG ReturnLength) {
    return ntsupGetLoadedModulesListEx(FALSE, ReturnLength,
 ...
 *
 */
PVOID ntsupGetLoadedModulesList2(_Out_opt_ PULONG ReturnLength) {
    return ntsupGetLoadedModulesListEx(TRUE, ReturnLength,
                                       (PNTSUPMEMALLOC)ntsupHeapAlloc,
                                       (PNTSUPMEMFREE)ntsupHeapFree);
}
 ...
 *
 */
PVOID ntsupGetSystemInfoEx(_In_ SYSTEM_INFORMATION_CLASS SystemInformationClass,
                           _Out_opt_ PULONG ReturnLength,
                           _In_ PNTSUPMEMALLOC AllocMem,
                           _In_ PNTSUPMEMFREE FreeMem) {
    PVOID buffer = NULL;
    ...
    while ((ntStatus = NtQuerySystemInformation(SystemInformationClass, buffer,
 ...
 *
 */
PVOID ntsupGetSystemInfo(_In_ SYSTEM_INFORMATION_CLASS SystemInformationClass,
                         _Out_opt_ PULONG ReturnLength) {
    return ntsupGetSystemInfoEx(SystemInformationClass, ReturnLength,
                                (PNTSUPMEMALLOC)ntsupHeapAlloc,
                                (PNTSUPMEMFREE)ntsupHeapFree);
}
 ...
 *
 */
BOOL ntsupResolveSymbolicLink(_In_opt_ HANDLE RootDirectoryHandle,
    ...
    InitializeObjectAttributes(&objectAttr, LinkName, OBJ_CASE_INSENSITIVE,
                               RootDirectoryHandle, NULL);

    ntStatus =
        NtOpenSymbolicLinkObject(&linkHandle, SYMBOLIC_LINK_QUERY, &objectAttr);
    ...
    ntStatus = NtQuerySymbolicLinkObject(linkHandle, &infoUString, NULL);
 ...
 *
 */
BOOL ntsupQueryThreadWin32StartAddress(_In_ HANDLE ThreadHandle,
    ...
    ntStatus = NtQueryInformationThread(
        ThreadHandle, ThreadQuerySetWin32StartAddress, &win32StartAddress,
        sizeof(ULONG_PTR), &returnLength);
 ...
 * Purpose:
 *
 * Open directory handle with DIRECTORY_QUERY access, with root directory
 * support.
 *
 */
_Success_(return) NTSTATUS
    ntsupOpenDirectoryEx(_Out_ PHANDLE DirectoryHandle,
    ...
    InitializeObjectAttributes(&objectAttrbutes, DirectoryName,
                               OBJ_CASE_INSENSITIVE, RootDirectoryHandle, NULL);

    ntStatus = NtOpenDirectoryObject(&directoryHandle, DesiredAccess,
                                     &objectAttrbutes);
 ...
 *
 */
NTSTATUS ntsupOpenDirectory(_Out_ PHANDLE DirectoryHandle,
                            _In_opt_ HANDLE RootDirectoryHandle,
    ...
    return ntsupOpenDirectoryEx(DirectoryHandle, RootDirectoryHandle, &usName,
                                DesiredAccess);
}
 ...
 *
 */
BOOL ntsupQueryProcessName(_In_ ULONG_PTR dwProcessId,
    ...
    do {
        ...
        if ((ULONG_PTR)List.Process->UniqueProcessId == dwProcessId) {
            _strncpy(Buffer, ccBuffer, List.Process->ImageName.Buffer,
 ...
 *
 */
BOOL ntsupQueryProcessEntryById(_In_ HANDLE UniqueProcessId,
                                _In_ PVOID ProcessList,
 ...
 *
 */
NTSTATUS ntsupQueryProcessImageFileNameByProcessId(_In_ HANDLE UniqueProcessId,
    ...
    do {
        processData.ImageName.Buffer =
            (PWSTR)AllocMem(processData.ImageName.MaximumLength);
        if (processData.ImageName.Buffer == NULL)
            return STATUS_INSUFFICIENT_RESOURCES;

        ntStatus = NtQuerySystemInformation(
            SystemProcessIdInformation, (PVOID)&processData,
            sizeof(SYSTEM_PROCESS_ID_INFORMATION), NULL);
...
NTSTATUS ntsupQuerySystemObjectInformationVariableSize(
    ...
    *Buffer = NULL;
    ...
    ntStatus = QueryRoutine(ObjectHandle, InformationClass, NULL, 0,
                            &returnLengthLocal);
    ...
    ntStatus = QueryRoutine(ObjectHandle, InformationClass, queryBuffer,
                            returnLengthLocal, &returnLengthLocal);
...
BOOLEAN ntsupQueryVsmProtectionInformation(
    ...
    if (pbDmaProtectionsAvailable)
        *pbDmaProtectionsAvailable = FALSE;
    if (pbDmaProtectionsInUse)
        *pbDmaProtectionsInUse = FALSE;
    if (pbHardwareMbecAvailable)
        *pbHardwareMbecAvailable = FALSE;
    if (pbApicVirtualizationAvailable)
        *pbApicVirtualizationAvailable = FALSE;

    RtlSecureZeroMemory(&svpi, sizeof(SYSTEM_VSM_PROTECTION_INFORMATION));

    ntStatus = NtQuerySystemInformation(
        SystemVsmProtectionInformation, &svpi,
        sizeof(SYSTEM_VSM_PROTECTION_INFORMATION), &returnLength);

    if (NT_SUCCESS(ntStatus)) {
        if (pbDmaProtectionsAvailable)
            *pbDmaProtectionsAvailable = svpi.DmaProtectionsAvailable;
        if (pbDmaProtectionsInUse)
            *pbDmaProtectionsInUse = svpi.DmaProtectionsInUse;
        if (pbHardwareMbecAvailable)
            *pbHardwareMbecAvailable = svpi.HardwareMbecAvailable;
        if (pbApicVirtualizationAvailable)
            *pbApicVirtualizationAvailable = svpi.ApicVirtualizationAvailable;
        return TRUE;
    } else {
        RtlSetLastWin32Error(RtlNtStatusToDosError(ntStatus));
    }
 ...
 *
 */
BOOLEAN ntsupQueryHVCIState(_Out_ PBOOLEAN pbHVCIEnabled,
    ...
    if (pbHVCIIUMEnabled)
        *pbHVCIIUMEnabled = FALSE;

    ci.Length = sizeof(ci);

    ntStatus = NtQuerySystemInformation(SystemCodeIntegrityInformation, &ci,
                                        sizeof(ci), &returnLength);
    ...
    if (NT_SUCCESS(ntStatus)) {
        ...
        if (pbHVCIIUMEnabled)
            *pbHVCIIUMEnabled = (ci.CodeIntegrityOptions &
                                 CODEINTEGRITY_OPTION_HVCI_IUM_ENABLED) > 0;
 ...
 *
 */
PVOID ntsupLookupImageSectionByName(_In_ CHAR* SectionName,
    ...
    SectionTableEntry =
        (PIMAGE_SECTION_HEADER)((PCHAR)NtHeaders + sizeof(ULONG) +
    ...
    while (i > 0) {
        if (_strncmp_a((CHAR*)SectionTableEntry->Name, SectionName,
                       SectionNameLength) == 0) {
            bFound = TRUE;
            break;
        }
 ...
 *
 */
PVOID ntsupFindPattern(_In_ CONST PBYTE Buffer,
 ...
 *
 */
DWORD ntsupFindPatternEx(_In_ PATTERN_SEARCH_PARAMS* SearchParams) {
    PBYTE p;
    DWORD c, i, n;
    BOOLEAN found;
    BYTE low, high;
    ...
    if ((SearchParams->PatternSize == 0) ||
        (SearchParams->PatternSize > SearchParams->BufferSize))
        return 0;
    ...
    for (n = 0, p = SearchParams->Buffer, c = 0; c <= bufferSize; ++p, ++c) {
        ...
        if (found) {
            if (SearchParams->Callback(p, SearchParams->PatternSize,
 ...
 *
 */
NTSTATUS ntsupOpenProcess(_In_ HANDLE UniqueProcessId,
                          _In_ ACCESS_MASK DesiredAccess,
                          _Out_ PHANDLE ProcessHandle) {
    NTSTATUS ntStatus;
    HANDLE processHandle = NULL;
    OBJECT_ATTRIBUTES objectAttributes =
        RTL_INIT_OBJECT_ATTRIBUTES((PUNICODE_STRING)NULL, 0);
    CLIENT_ID ClientId;

    ClientId.UniqueProcess = UniqueProcessId;
    ClientId.UniqueThread = NULL;

    ntStatus = NtOpenProcess(&processHandle, DesiredAccess, &objectAttributes,
                             &ClientId);
 ...
 *
 */
NTSTATUS ntsupOpenThread(_In_ PCLIENT_ID ClientId,
                         _In_ ACCESS_MASK DesiredAccess,
 ...
 *
 */
NTSTATUS ntsupCICustomKernelSignersAllowed(_Out_ PBOOLEAN bAllowed) {
    NTSTATUS ntStatus;
    ULONG uLicense = 0, dataSize;
    UNICODE_STRING usLicenseValue = RTL_CONSTANT_STRING(
        L"CodeIntegrity-AllowConfigurablePolicy-CustomKernelSigners");

    *bAllowed = FALSE;

    ntStatus = NtQueryLicenseValue(&usLicenseValue, NULL, (PVOID)&uLicense,
                                   sizeof(DWORD), &dataSize);
 ...
 *
 */
NTSTATUS ntsupPrivilegeEnabled(_In_ HANDLE ClientToken,
 ...
 *
 */
LPWSTR ntsupQueryEnvironmentVariableOffset(_In_ PUNICODE_STRING Value) {
    UNICODE_STRING str1;
 ...
 *
 */
DWORD ntsupExpandEnvironmentStrings(_In_ LPCWSTR lpSrc,
                                    _Out_writes_to_opt_(nSize, return)
                                        LPWSTR lpDst,
                                    _In_ DWORD nSize) {
    NTSTATUS ntStatus;
    ...
    ntStatus = RtlExpandEnvironmentStrings(
        NULL, (PWSTR)lpSrc, srcLength, (PWSTR)lpDst, dstLength, &returnLength);
 ...
 * Purpose:
 *
 * pbInteractiveSid will be set to TRUE if current user has interactive sid,
 * FALSE otherwise.
 *
 * Function return operation status code.
 *
 */
NTSTATUS ntsupIsUserHasInteractiveSid(_In_ HANDLE hToken,
    ...
    do {
        ntStatus = NtQueryInformationToken(hToken, TokenGroups, NULL, 0,
                                           &neededLength);

        if (ntStatus != STATUS_BUFFER_TOO_SMALL)
            break;

        groupInfo = (PTOKEN_GROUPS)RtlAllocateHeap(heapHandle, HEAP_ZERO_MEMORY,
                                                   neededLength);

        if (groupInfo == NULL)
            break;

        ntStatus = NtQueryInformationToken(hToken, TokenGroups, groupInfo,
        ...
        ntStatus =
            RtlAllocateAndInitializeSid(&SidAuth, 1, SECURITY_INTERACTIVE_RID,
        ...
        for (i = 0; i < groupInfo->GroupCount; i++) {
            if (RtlEqualSid(pInteractiveSid, groupInfo->Groups[i].Sid)) {
 ...
 * Purpose:
 *
 * pbResult will be set to TRUE if current account is run by system user, FALSE
 * otherwise.
 *
 * Function return operation status code.
 *
 */
NTSTATUS ntsupIsLocalSystem(_Out_ PBOOL pbResult) {
    BOOL bResult = FALSE;
    ...
    ntStatus =
        NtOpenProcessToken(NtCurrentProcess(), TOKEN_QUERY, &tokenHandle);
    ...
    if (NT_SUCCESS(ntStatus)) {
        ...
        if (ntStatus == STATUS_BUFFER_TOO_SMALL) {
            ptu = (PTOKEN_USER)RtlAllocateHeap(heapHandle, HEAP_ZERO_MEMORY,
                                               neededLength);

            if (ptu) {
                ntStatus = NtQueryInformationToken(tokenHandle, TokenUser, ptu,
                                                   neededLength, &neededLength);

                if (NT_SUCCESS(ntStatus)) {
                    ntStatus = RtlAllocateAndInitializeSid(
                        &ntAuthority, 1, SECURITY_LOCAL_SYSTEM_RID, 0, 0, 0, 0,
                        0, 0, 0, &systemSid);
 ...
 *
 */
BOOL ntsupGetProcessElevationType(_In_opt_ HANDLE ProcessHandle,
    ...
    if (NT_SUCCESS(ntStatus)) {
        ntStatus = NtQueryInformationToken(
            tokenHandle, TokenElevationType, &tokenType,
            sizeof(TOKEN_ELEVATION_TYPE), &returnedLength);
 ...
 *
 */
NTSTATUS ntsupIsProcessElevated(_In_ ULONG ProcessId, _Out_ PBOOL Elevated) {
    NTSTATUS ntStatus;
    ...
    ntStatus = ntsupOpenProcess(UlongToHandle(ProcessId), MAXIMUM_ALLOWED,
                                &processHandle);
    ...
    if (NT_SUCCESS(ntStatus)) {
        ...
        if (NT_SUCCESS(ntStatus)) {
            ...
            ntStatus = NtQueryInformationToken(
                tokenHandle, TokenElevation, &tokenInfo,
                sizeof(TOKEN_ELEVATION), &returnedLength);
 ...
 *
 */
VOID ntsupPurgeSystemCache(VOID) {
    ...
    if (ntsupEnablePrivilege(SE_INCREASE_QUOTA_PRIVILEGE, TRUE)) {
        ...
        NtSetSystemInformation(SystemFileCacheInformation, (PVOID)&sfc,
                               sizeof(sfc));
    }
    ...
    if (ntsupEnablePrivilege(SE_PROF_SINGLE_PROCESS_PRIVILEGE, TRUE)) {
        ...
        NtSetSystemInformation(SystemMemoryListInformation, (PVOID)&smlc,
                               sizeof(smlc));
    }
}
 ...
 *
 */
NTSTATUS ntsupGetProcessDebugObject(_In_ HANDLE ProcessHandle,
                                    _Out_ PHANDLE DebugObjectHandle) {
    return NtQueryInformationProcess(ProcessHandle, ProcessDebugObjectHandle,
                                     DebugObjectHandle, sizeof(HANDLE), NULL);
}
 ...
 *
 */
PBYTE ntsupQueryResourceData(_In_ ULONG_PTR ResourceId,
    ...
    if (DllHandle != NULL) {
        ...
        ntStatus =
            LdrFindResource_U(DllHandle, (ULONG_PTR*)&idPath, 3, &dataEntry);
        if (NT_SUCCESS(ntStatus)) {
            ntStatus = LdrAccessResource(DllHandle, dataEntry, (PVOID*)&dataPtr,
                                         &dataSize);
            if (NT_SUCCESS(ntStatus)) {
                if (DataSize) {
                    *DataSize = dataSize;
 ...
 *
 */
NTSTATUS ntsupEnableWow64Redirection(_In_ BOOLEAN bEnable) {
 ...
 */
NTSTATUS NTAPI
ntsupDetectObjectCallback(_In_ POBJECT_DIRECTORY_INFORMATION Entry,
 ...
 *
 */
NTSTATUS NTAPI ntsupEnumSystemObjects(_In_opt_ LPCWSTR pwszRootDirectory,
    ...
    if (pwszRootDirectory != NULL) {
        ...
        InitializeObjectAttributes(&attr, &sname, OBJ_CASE_INSENSITIVE, NULL,
                                   NULL);
    ...
    do {
        rlen = 0;
        status = NtQueryDirectoryObject(hDirectory, NULL, 0, TRUE, FALSE, &ctx,
                                        &rlen);
        ...
        status = NtQueryDirectoryObject(hDirectory, objinf, rlen, TRUE, FALSE,
                                        &ctx, &rlen);
        if (!NT_SUCCESS(status)) {
 ...
 *
 */
BOOLEAN ntsupIsObjectExists(_In_ LPCWSTR RootDirectory,
    ...
    return NT_SUCCESS(ntsupEnumSystemObjects(
        RootDirectory, NULL, ntsupDetectObjectCallback, &Param));
}
 ...
 *
 */
BOOLEAN ntsupUserIsFullAdmin(VOID) {
    BOOLEAN bResult = FALSE;
    HANDLE hToken = NULL;
    NTSTATUS status;
    DWORD i, Attributes;
    ...
    if (hToken) {
        if (NT_SUCCESS(RtlAllocateAndInitializeSid(
                ...
                &AdministratorsGroup))) {
            status = ntsupQueryTokenInformation(
                hToken, TokenGroups, &pTkGroups, &ReturnLength,
                (PNTSUPMEMALLOC)ntsupHeapAlloc, (PNTSUPMEMFREE)ntsupHeapFree);

            if (NT_SUCCESS(status)) {
                for (i = 0; i < pTkGroups->GroupCount; i++) {
                    Attributes = pTkGroups->Groups[i].Attributes;

                    if (RtlEqualSid(AdministratorsGroup,
                                    pTkGroups->Groups[i].Sid))
 ...
 *
 */
NTSTATUS ntsupHashImageSections(_In_ PVOID ImageBase,
                                _In_ SIZE_T ImageSize,  // Size of image mapping
                                _Out_writes_bytes_(HashBufferSize)
                                    PBYTE HashBuffer,
    ...
    //
    if (!NT_SUCCESS(RtlImageNtHeaderEx(0, ImageBase, ImageSize,
                                       (PIMAGE_NT_HEADERS*)&ntHeaders)) ||
        ntHeaders == NULL) {
    ...
    for (i = 0; i < numberOfSections; i++, sectionHeader++) {
        ...
        if (ImageType == ImageTypeLoaded) {
            ...
            maxVirtualSize = ntHeaders->OptionalHeader.SizeOfImage -
