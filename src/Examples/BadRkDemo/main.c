...
typedef NTSTATUS(NTAPI* pfnDispatch)(_In_ struct _DEVICE_OBJECT* DeviceObject,
...
NTSTATUS ModifyPsLoadedModuleList(_In_ DEVICE_OBJECT* DeviceObject) {
    DRIVER_OBJECT* DriverObject = DeviceObject->DriverObject;
    KLDR_DATA_TABLE_ENTRY* LoaderSection =
        (KLDR_DATA_TABLE_ENTRY*)DriverObject->DriverSection;
...
NTSTATUS NtfsFsdCreateHook(_In_ PDEVICE_OBJECT DeviceObject, _In_ PIRP Irp) {
...
#pragma warning(push)
#pragma warning(disable : 28175)  // private member of struct
NTSTATUS ModifyDriverObject(VOID) {
    NTSTATUS Status;
    ...
    //
    Status = ObReferenceObjectByName(&fsdName, OBJ_CASE_INSENSITIVE, NULL, 0,
...
NTSTATUS SetNotifyFromPool(VOID) {
    ...
    if (CodeBuffer) {
        ...
        status =
            PsSetLoadImageNotifyRoutine((PLOAD_IMAGE_NOTIFY_ROUTINE)CodeBuffer);

        DbgPrintEx(
            DPFLTR_DEFAULT_ID, DPFLTR_INFO_LEVEL,
...
NTSTATUS CreateDispatch(_In_ struct _DEVICE_OBJECT* DeviceObject,
    ...
    DbgPrintEx(DPFLTR_DEFAULT_ID, DPFLTR_INFO_LEVEL, "[PGDemo] %s Create\r\n",
               __FUNCTION__);
...
NTSTATUS CloseDispatch(_In_ struct _DEVICE_OBJECT* DeviceObject,
    ...
    DbgPrintEx(DPFLTR_DEFAULT_ID, DPFLTR_INFO_LEVEL, "[PGDemo] %s Close\r\n",
               __FUNCTION__);
...
VOID BeaconRoutine(_In_ PVOID StartContext) {
...
NTSTATUS DriverEntry(_In_ struct _DRIVER_OBJECT* DriverObject,
    ...
    DbgPrintEx(DPFLTR_DEFAULT_ID, DPFLTR_INFO_LEVEL, "[PGDemo] %s\r\n",
               __FUNCTION__);
    ...
    status = IoCreateDeviceSecure(
        DriverObject, 0, &DevName, FILE_DEVICE_UNKNOWN, FILE_DEVICE_SECURE_OPEN,
        FALSE, &SddlString, (LPCGUID)&GUID_PGDEMO, &devobj);
